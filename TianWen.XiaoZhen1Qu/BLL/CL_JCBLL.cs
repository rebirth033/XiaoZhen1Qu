            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        DAO.Save(CL_JCJBXX);
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", CL_JCJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
        ...
        public object LoadCL_JCJBXX(string CL_JCJBXXID)
        {
            try
            {
                ...
                if (CL_JCJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { CL_JCJBXX = CL_JCJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(CL_JCJBXX.JCXXID) } };
                }
                else
                {
