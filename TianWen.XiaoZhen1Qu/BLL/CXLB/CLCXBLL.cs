    ...
    public class CLCXBLL : BaseBLL, ICLCXBLL
    {
        ...
        public object LoadCLXX(string TYPE, string Condition, string PageIndex, string PageSize, string OrderColumn, string OrderType, string XZQDM)
        {
            try
            {
                ...
                if (TYPE == "CLXX_JC")
                {
                    dt = DAO.Repository.GetDataTable(string.Format("select a.*,b.*,c.*,d.* from jcxx a,cl_jcjbxx b,codes_cl_jc c,codes_cl_jc_xxxx d where a.jcxxid = b.jcxxid and b.ksid = c.codeid and c.codeid = d.id and status = 1 and xzqdm = {0} " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType), XZQDM));
        ...
        public object LoadCLXX(string TYPE, string ID)
        {
            try
            {
                ...
                if (TYPE == "CLXX_JC") 
                {
                    dt = DAO.Repository.GetDataTable(string.Format("select a.*,b.*,c.*,d.* from jcxx a,cl_jcjbxx b,codes_cl_jc c,codes_cl_jc_xxxx d where a.jcxxid = b.jcxxid and b.ksid = c.codeid and c.codeid = d.id and b.id = '{0}'  order by zxgxsj desc", ID));
                    List<CL_JCView> list = ConvertHelper.DataTableToList<CL_JCView>(dt);
