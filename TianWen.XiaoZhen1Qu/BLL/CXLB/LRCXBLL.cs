...
namespace TianWen.XiaoZhen1Qu.BLL
{
    ...
    {
        //加载丽人信息
        public object LoadLRXX(string TYPE, string Condition, string PageIndex, string PageSize, string OrderColumn, string OrderType)
        {
            try
            {
                ...
                if (TYPE == "LRXX_MTSS")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,lr_mtssjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType));
                ...
                if (TYPE == "LRXX_MRHF")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,lr_mrhfjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType));
                    List<LR_MTSSView> list = ConvertHelper.DataTableToList<LR_MTSSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "LRXX_SPA")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,lr_spajbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType));
                    List<LR_MTSSView> list = ConvertHelper.DataTableToList<LR_MTSSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "LRXX_WD")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,lr_wdjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType));
                    List<LR_MTSSView> list = ConvertHelper.DataTableToList<LR_MTSSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "LRXX_YJ")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,lr_yjjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType));
                    List<LR_MTSSView> list = ConvertHelper.DataTableToList<LR_MTSSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "LRXX_MFHF")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,lr_mfhfjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType));
                    List<LR_MTSSView> list = ConvertHelper.DataTableToList<LR_MTSSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "LRXX_MJ")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,lr_mjjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType));
                    List<LR_MTSSView> list = ConvertHelper.DataTableToList<LR_MTSSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "LRXX_WS")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,lr_wsjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType));
                    List<LR_MTSSView> list = ConvertHelper.DataTableToList<LR_MTSSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "LRXX_KQHL")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,lr_kqhljbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType));
                    List<LR_MTSSView> list = ConvertHelper.DataTableToList<LR_MTSSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "LRXX_TJ")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,lr_tjjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + GetOrder(OrderColumn, OrderType));
                    List<LR_MTSSView> list = ConvertHelper.DataTableToList<LR_MTSSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
