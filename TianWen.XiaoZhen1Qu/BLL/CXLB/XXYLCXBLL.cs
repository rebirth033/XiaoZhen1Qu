...
namespace TianWen.XiaoZhen1Qu.BLL
{
    ...
    {
        //加载休闲娱乐列表信息
        public object LoadXXYLXX(string TYPE, string Condition, string PageIndex, string PageSize)
        {
            try
            {
                ...
                if (TYPE == "XXYLXX_YDJS")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,xxyl_ydjsjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + " order by zxgxsj desc");
                ...
                if (TYPE == "XXYLXX_YDJB")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,xxyl_ydjbjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + " order by zxgxsj desc");
                    List<XXYL_YDJSView> list = ConvertHelper.DataTableToList<XXYL_YDJSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "XXYLXX_KTV")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,xxyl_ktvjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + " order by zxgxsj desc");
                    List<XXYL_YDJSView> list = ConvertHelper.DataTableToList<XXYL_YDJSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "XXYLXX_HW")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,xxyl_hwjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + " order by zxgxsj desc");
                    List<XXYL_YDJSView> list = ConvertHelper.DataTableToList<XXYL_YDJSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "XXYLXX_XYWQ")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,xxyl_xywqjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + " order by zxgxsj desc");
                    List<XXYL_YDJSView> list = ConvertHelper.DataTableToList<XXYL_YDJSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "XXYLXX_ZLAM")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,xxyl_zlamjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + " order by zxgxsj desc");
                    List<XXYL_YDJSView> list = ConvertHelper.DataTableToList<XXYL_YDJSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "XXYLXX_TQT")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,xxyl_tqtjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + " order by zxgxsj desc");
                    List<XXYL_YDJSView> list = ConvertHelper.DataTableToList<XXYL_YDJSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "XXYLXX_QPZY")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,xxyl_qpzyjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + " order by zxgxsj desc");
                    List<XXYL_YDJSView> list = ConvertHelper.DataTableToList<XXYL_YDJSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,xxyl_diysgfjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + " order by zxgxsj desc");
                    List<XXYL_YDJSView> list = ConvertHelper.DataTableToList<XXYL_YDJSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
                ...
                if (TYPE == "XXYLXX_HPG")
                {
                    dt = DAO.Repository.GetDataTable("select a.*,b.* from jcxx a,xxyl_hpgjbxx b where a.jcxxid = b.jcxxid " + GetConditin(Condition) + " order by zxgxsj desc");
                    List<XXYL_YDJSView> list = ConvertHelper.DataTableToList<XXYL_YDJSView>(dt);
                    int PageCount = (list.Count + int.Parse(PageSize) - 1) / int.Parse(PageSize);
                    int TotalCount = list.Count;
                    var listnew = from p in list.Skip((int.Parse(PageIndex) - 1) * int.Parse(PageSize)).Take(int.Parse(PageSize)) select p;
