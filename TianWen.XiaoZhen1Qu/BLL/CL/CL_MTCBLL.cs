            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        DAO.Save(CL_MTCJBXX);
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", CL_MTCJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
        ...
        public object LoadCL_MTCJBXX(string CL_MTCJBXXID)
        {
            try
            {
                ...
                if (CL_MTCJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { CL_MTCJBXX = CL_MTCJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(CL_MTCJBXX.JCXXID) } };
                }
                else
                {
