            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", ZSJM_JXJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
        ...
        public object LoadZSJM_JXJBXX(string ZSJM_JXJBXXID)
        {
            try
            {
                ...
                if (ZSJM_JXJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { ZSJM_JXJBXX = ZSJM_JXJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(ZSJM_JXJBXX.JCXXID) } };
                }
                else
                {
