            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", ZSJM_SHFWJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = ZSJM_SHFWJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadZSJM_SHFWJBXX(string ZSJM_SHFWJBXXID)
        {
            try
            {
                ...
                if (ZSJM_SHFWJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { ZSJM_SHFWJBXX = ZSJM_SHFWJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(ZSJM_SHFWJBXX.JCXXID) } };
                }
                else
                {
