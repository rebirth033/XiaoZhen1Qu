            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", ZSJM_WTMYETJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = ZSJM_WTMYETJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadZSJM_WTMYETJBXX(string ZSJM_WTMYETJBXXID)
        {
            try
            {
                ...
                if (ZSJM_WTMYETJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { ZSJM_WTMYETJBXX = ZSJM_WTMYETJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(ZSJM_WTMYETJBXX.JCXXID) } };
                }
                else
                {
