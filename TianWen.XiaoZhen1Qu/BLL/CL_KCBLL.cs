            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        DAO.Save(CL_KCJBXX);
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", CL_KCJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
        ...
        public object LoadCL_KCJBXX(string CL_KCJBXXID)
        {
            try
            {
                ...
                if (CL_KCJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { CL_KCJBXX = CL_KCJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(CL_KCJBXX.JCXXID) } };
                }
                else
                {
