            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        DAO.Save(CL_HCJBXX);
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", CL_HCJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
        ...
        public object LoadCL_HCJBXX(string CL_HCJBXXID)
        {
            try
            {
                ...
                if (CL_HCJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { CL_HCJBXX = CL_HCJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(CL_HCJBXX.JCXXID) } };
                }
                else
                {
