            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", ES_WHYL_YSPSCPJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = ES_WHYL_YSPSCPJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadES_WHYL_YSPSCPJBXX(string ES_WHYL_YSPSCPJBXXID)
        {
            try
            {
                ...
                if (ES_WHYL_YSPSCPJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { ES_WHYL_YSPSCPJBXX = ES_WHYL_YSPSCPJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(ES_WHYL_YSPSCPJBXX.JCXXID) } };
                }
                else
                {
