            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", ES_QTES_ESSBJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = ES_QTES_ESSBJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadES_QTES_ESSBJBXX(string ES_QTES_ESSBJBXXID)
        {
            try
            {
                ...
                if (ES_QTES_ESSBJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { ES_QTES_ESSBJBXX = ES_QTES_ESSBJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(ES_QTES_ESSBJBXX.JCXXID) } };
                }
                else
                {
