            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", XXYL_YDJSJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = XXYL_YDJSJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadXXYL_YDJSJBXX(string XXYL_YDJSJBXXID)
        {
            try
            {
                ...
                if (XXYL_YDJSJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { XXYL_YDJSJBXX = XXYL_YDJSJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(XXYL_YDJSJBXX.JCXXID) } };
                }
                else
                {
