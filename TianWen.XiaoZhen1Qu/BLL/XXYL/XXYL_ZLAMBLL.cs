            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", XXYL_ZLAMJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = XXYL_ZLAMJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadXXYL_ZLAMJBXX(string XXYL_ZLAMJBXXID)
        {
            try
            {
                ...
                if (XXYL_ZLAMJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { XXYL_ZLAMJBXX = XXYL_ZLAMJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(XXYL_ZLAMJBXX.JCXXID) } };
                }
                else
                {
