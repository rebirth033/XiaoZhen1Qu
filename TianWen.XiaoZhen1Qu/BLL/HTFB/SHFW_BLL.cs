...
namespace TianWen.XiaoZhen1Qu.BLL
{
    public class SHFW_BLL : BaseBLL, ISHFW_BLL
    {
        ...
        public object SaveSHFW_KSXSHSJBXX(JCXX jcxx, SHFW_KSXSHSJBXX SHFW_KSXSHSJBXX, List<PHOTOS> photos)
        {
            DataTable dt = DAO.Repository.GetDataTable(string.Format("SELECT * FROM SHFW_KSXSHSJBXX WHERE ID='{0}'", SHFW_KSXSHSJBXX.ID));
            using (ITransaction transaction = DAO.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        SavePhotos(photos, dt.Rows[0]["JCXXID"].ToString());
                        SHFW_KSXSHSJBXX.JCXXID = dt.Rows[0]["JCXXID"].ToString();
                        jcxx.JCXXID = dt.Rows[0]["JCXXID"].ToString();
                        DAO.Update(jcxx);
                        DAO.Update(SHFW_KSXSHSJBXX);
                        transaction.Commit();
                        return new { Result = EnResultType.Success, Message = "修改成功!", Value = new { JCXXID = jcxx.JCXXID, ID = SHFW_KSXSHSJBXX.ID } };
                    }
                    else
                    {
                        SavePhotos(photos, jcxx.JCXXID);
                        SHFW_KSXSHSJBXX.JCXXID = jcxx.JCXXID;
                        DAO.Save(jcxx);
                        DAO.Save(SHFW_KSXSHSJBXX);
                        transaction.Commit();
                        return new { Result = EnResultType.Success, Message = "新增成功!", Value = new { JCXXID = jcxx.JCXXID, ID = SHFW_KSXSHSJBXX.ID } };
                    }
                }
                catch (Exception ex)
                {
                    transaction.Rollback();
                    LoggerManager.Error("SHFW_KSXSHSJBXXBLL", "保存失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!");
                    return new { Result = EnResultType.Failed, Message = "保存失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!", Type = 3 };
                }
            }
        }

        public object LoadSHFW_KSXSHSJBXX(string ID)
        {
            try
            {
                SHFW_KSXSHSJBXX SHFW_KSXSHSJBXX = DAO.GetObjectByID<SHFW_KSXSHSJBXX>(ID);
                if (SHFW_KSXSHSJBXX != null)
                {
                    JCXX jcxx = GetJCXXByID(SHFW_KSXSHSJBXX.JCXXID);
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { SHFW_KSXSHSJBXX = SHFW_KSXSHSJBXX, BCMSString = BinaryHelper.BinaryToString(SHFW_KSXSHSJBXX.BCMS), JCXX = jcxx, Photos = GetPhtosByJCXXID(SHFW_KSXSHSJBXX.JCXXID) } };
                }
                else
                {
                    return new { Result = EnResultType.Failed, Message = "不存在" };
            ...
            {
                LoggerManager.Error("SHFW_KSXSHSJBXXBLL", "载入失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!");
                return new
                {
                    Result = EnResultType.Failed,
                    Message = "载入失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!"
        ...
        public object SaveSHFW_ESHSJBXX(JCXX jcxx, SHFW_ESHSJBXX SHFW_ESHSJBXX, List<PHOTOS> photos)
        {
            DataTable dt = DAO.Repository.GetDataTable(string.Format("SELECT * FROM SHFW_ESHSJBXX WHERE ID='{0}'", SHFW_ESHSJBXX.ID));
            using (ITransaction transaction = DAO.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        SavePhotos(photos, dt.Rows[0]["JCXXID"].ToString());
                        SHFW_ESHSJBXX.JCXXID = dt.Rows[0]["JCXXID"].ToString();
                        jcxx.JCXXID = dt.Rows[0]["JCXXID"].ToString();
                        DAO.Update(jcxx);
                        DAO.Update(SHFW_ESHSJBXX);
                        transaction.Commit();
                        return new { Result = EnResultType.Success, Message = "修改成功!", Value = new { JCXXID = jcxx.JCXXID, ID = SHFW_ESHSJBXX.ID } };
                    }
                    else
                    {
                        SavePhotos(photos, jcxx.JCXXID);
                        SHFW_ESHSJBXX.JCXXID = jcxx.JCXXID;
                        DAO.Save(jcxx);
                        DAO.Save(SHFW_ESHSJBXX);
                        transaction.Commit();
                        return new { Result = EnResultType.Success, Message = "新增成功!", Value = new { JCXXID = jcxx.JCXXID, ID = SHFW_ESHSJBXX.ID } };
                    }
                }
                catch (Exception ex)
                {
        ...
        public object LoadSHFW_ESHSJBXX(string ID)
        {
            try
            {
                SHFW_ESHSJBXX SHFW_ESHSJBXX = DAO.GetObjectByID<SHFW_ESHSJBXX>(ID);
                if (SHFW_ESHSJBXX != null)
                {
                    JCXX jcxx = GetJCXXByID(SHFW_ESHSJBXX.JCXXID);
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { SHFW_ESHSJBXX = SHFW_ESHSJBXX, BCMSString = BinaryHelper.BinaryToString(SHFW_ESHSJBXX.BCMS), JCXX = jcxx, Photos = GetPhtosByJCXXID(SHFW_ESHSJBXX.JCXXID) } };
                }
                else
                {
                    return new { Result = EnResultType.Failed, Message = "不存在" };
