...
namespace TianWen.XiaoZhen1Qu.BLL.HTFB
{
    public class CL_BLL : BaseBLL, ICL_BLL
    {
        ...
        public object SaveCL_QCPJJBXX(JCXX jcxx, CL_QCPJJBXX CL_QCPJJBXX, List<PHOTOS> photos)
        {
            DataTable dt = DAO.Repository.GetDataTable(string.Format("SELECT * FROM CL_QCPJJBXX WHERE ID='{0}'", CL_QCPJJBXX.ID));
            using (ITransaction transaction = DAO.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        SavePhotos(photos, dt.Rows[0]["JCXXID"].ToString());
                        CL_QCPJJBXX.JCXXID = dt.Rows[0]["JCXXID"].ToString();
                        jcxx.JCXXID = dt.Rows[0]["JCXXID"].ToString();
                        DAO.Update(jcxx);
                        DAO.Update(CL_QCPJJBXX);
                        transaction.Commit();
                        return new { Result = EnResultType.Success, Message = "修改成功!", Value = new { JCXXID = jcxx.JCXXID, ID = CL_QCPJJBXX.ID } };
                    }
                    else
                    {
                        SavePhotos(photos, jcxx.JCXXID);
                        CL_QCPJJBXX.JCXXID = jcxx.JCXXID;
                        DAO.Save(jcxx);
                        DAO.Save(CL_QCPJJBXX);
                        transaction.Commit();
                        return new { Result = EnResultType.Success, Message = "新增成功!", Value = new { JCXXID = jcxx.JCXXID, ID = CL_QCPJJBXX.ID } };
                    }
                }
                catch (Exception ex)
                {
                    transaction.Rollback();
                    LoggerManager.Error("CL_QCPJJBXXBLL", "保存失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!");
                    return new { Result = EnResultType.Failed, Message = "保存失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!", Type = 3 };
                }
            }
        }

        public object LoadCL_QCPJJBXX(string ID)
        {
            try
            {
                CL_QCPJJBXX CL_QCPJJBXX = DAO.GetObjectByID<CL_QCPJJBXX>(ID);
                if (CL_QCPJJBXX != null)
                {
                    JCXX jcxx = GetJCXXByID(CL_QCPJJBXX.JCXXID);
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { CL_QCPJJBXX = CL_QCPJJBXX, BCMSString = BinaryHelper.BinaryToString(CL_QCPJJBXX.BCMS), JCXX = jcxx, Photos = GetPhtosByJCXXID(CL_QCPJJBXX.JCXXID) } };
                }
                else
                {
                    return new { Result = EnResultType.Failed, Message = "不存在" };
            ...
            {
                LoggerManager.Error("CL_QCPJJBXXBLL", "载入失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!");
                return new
                {
                    Result = EnResultType.Failed,
                    Message = "载入失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!"
