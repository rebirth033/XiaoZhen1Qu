...
namespace TianWen.XiaoZhen1Qu.BLL
{
    public class ES_BLL : BaseBLL, IES_BLL
    {
        ...
        public object SaveES_PWKQ_MPKQJBXX(JCXX jcxx, ES_PWKQ_MPKQJBXX ES_PWKQ_MPKQJBXX)
        {
            DataTable dt = DAO.Repository.GetDataTable(string.Format("SELECT * FROM ES_PWKQ_MPKQJBXX WHERE ID='{0}'", ES_PWKQ_MPKQJBXX.ID));
            using (ITransaction transaction = DAO.BeginTransaction())
            {
                try
                {
                    if (dt.Rows.Count > 0)
                    {
                        ES_PWKQ_MPKQJBXX.JCXXID = dt.Rows[0]["JCXXID"].ToString();
                        jcxx.JCXXID = dt.Rows[0]["JCXXID"].ToString();
                        DAO.Update(jcxx);
                        DAO.Update(ES_PWKQ_MPKQJBXX);
                        transaction.Commit();
                        return new { Result = EnResultType.Success, Message = "修改成功!", Value = new { JCXXID = jcxx.JCXXID, ID = ES_PWKQ_MPKQJBXX.ID } };
                    }
                    else
                    {
                        ES_PWKQ_MPKQJBXX.JCXXID = jcxx.JCXXID;
                        DAO.Save(jcxx);
                        DAO.Save(ES_PWKQ_MPKQJBXX);
                        transaction.Commit();
                        return new { Result = EnResultType.Success, Message = "新增成功!", Value = new { JCXXID = jcxx.JCXXID, ID = ES_PWKQ_MPKQJBXX.ID } };
                    }
                }
                catch (Exception ex)
                {
                    transaction.Rollback();
                    LoggerManager.Error("ES_PWKQ_MPKQJBXXBLL", "保存失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!");
                    return new { Result = EnResultType.Failed, Message = "保存失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!", Type = 3 };
                }
            }
        }

        public object LoadES_PWKQ_MPKQJBXX(string ID)
        {
            try
            {
                ES_PWKQ_MPKQJBXX ES_PWKQ_MPKQJBXX = DAO.GetObjectByID<ES_PWKQ_MPKQJBXX>(ID);
                if (ES_PWKQ_MPKQJBXX != null)
                {
                    JCXX jcxx = GetJCXXByID(ES_PWKQ_MPKQJBXX.JCXXID);
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { ES_PWKQ_MPKQJBXX = ES_PWKQ_MPKQJBXX, BCMSString = BinaryHelper.BinaryToString(ES_PWKQ_MPKQJBXX.BCMS), JCXX = jcxx, Photos = GetPhtosByJCXXID(ES_PWKQ_MPKQJBXX.JCXXID) } };
                }
                else
                {
                    return new { Result = EnResultType.Failed, Message = "不存在" };
            ...
            {
                LoggerManager.Error("ES_PWKQ_MPKQJBXXBLL", "载入失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!");
                return new
                {
                    Result = EnResultType.Failed,
                    Message = "载入失败【" + ex.Message + "\r\n" + ex.StackTrace + "】!"
