            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", CW_CWYPSPJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = CW_CWYPSPJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadCW_CWYPSPJBXX(string CW_CWYPSPJBXXID)
        {
            try
            {
                ...
                if (CW_CWYPSPJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { CW_CWYPSPJBXX = CW_CWYPSPJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(CW_CWYPSPJBXX.JCXXID) } };
                }
                else
                {
