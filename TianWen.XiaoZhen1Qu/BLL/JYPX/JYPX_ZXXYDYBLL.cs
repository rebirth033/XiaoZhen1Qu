            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", JYPX_ZXXYDYJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = JYPX_ZXXYDYJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadJYPX_ZXXYDYJBXX(string JYPX_ZXXYDYJBXXID)
        {
            try
            {
                ...
                if (JYPX_ZXXYDYJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { JYPX_ZXXYDYJBXX = JYPX_ZXXYDYJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(JYPX_ZXXYDYJBXX.JCXXID) } };
                }
                else
                {
