            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", JYPX_XLJYJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = JYPX_XLJYJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadJYPX_XLJYJBXX(string JYPX_XLJYJBXXID)
        {
            try
            {
                ...
                if (JYPX_XLJYJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { JYPX_XLJYJBXX = JYPX_XLJYJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(JYPX_XLJYJBXX.JCXXID) } };
                }
                else
                {
