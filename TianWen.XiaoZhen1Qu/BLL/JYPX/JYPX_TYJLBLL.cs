            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", JYPX_TYJLJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = JYPX_TYJLJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadJYPX_TYJLJBXX(string JYPX_TYJLJBXXID)
        {
            try
            {
                ...
                if (JYPX_TYJLJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { JYPX_TYJLJBXX = JYPX_TYJLJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(JYPX_TYJLJBXX.JCXXID) } };
                }
                else
                {
