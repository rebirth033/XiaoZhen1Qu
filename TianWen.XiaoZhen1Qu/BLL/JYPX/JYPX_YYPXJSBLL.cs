            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", JYPX_YYPXJSJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = JYPX_YYPXJSJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadJYPX_YYPXJSJBXX(string JYPX_YYPXJSJBXXID)
        {
            try
            {
                ...
                if (JYPX_YYPXJSJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { JYPX_YYPXJSJBXX = JYPX_YYPXJSJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(JYPX_YYPXJSJBXX.JCXXID) } };
                }
                else
                {
