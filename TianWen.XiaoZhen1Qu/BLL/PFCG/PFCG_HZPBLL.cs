            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", PFCG_HZPJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = PFCG_HZPJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadPFCG_HZPJBXX(string PFCG_HZPJBXXID)
        {
            try
            {
                ...
                if (PFCG_HZPJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { PFCG_HZPJBXX = PFCG_HZPJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(PFCG_HZPJBXX.JCXXID) } };
                }
                else
                {
