            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", PFCG_HWYDJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = PFCG_HWYDJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadPFCG_HWYDJBXX(string PFCG_HWYDJBXXID)
        {
            try
            {
                ...
                if (PFCG_HWYDJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { PFCG_HWYDJBXX = PFCG_HWYDJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(PFCG_HWYDJBXX.JCXXID) } };
                }
                else
                {
