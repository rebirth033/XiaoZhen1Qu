            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", PFCG_MYWJJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = PFCG_MYWJJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadPFCG_MYWJJBXX(string PFCG_MYWJJBXXID)
        {
            try
            {
                ...
                if (PFCG_MYWJJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { PFCG_MYWJJBXX = PFCG_MYWJJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(PFCG_MYWJJBXX.JCXXID) } };
                }
                else
                {
