            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", PFCG_LPJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
        ...
        public object LoadPFCG_LPJBXX(string PFCG_LPJBXXID)
        {
            try
            {
                ...
                if (PFCG_LPJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { PFCG_LPJBXX = PFCG_LPJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(PFCG_LPJBXX.JCXXID) } };
                }
                else
                {
