            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", PFCG_BZJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
        ...
        public object LoadPFCG_BZJBXX(string PFCG_BZJBXXID)
        {
            try
            {
                ...
                if (PFCG_BZJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { PFCG_BZJBXX = PFCG_BZJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(PFCG_BZJBXX.JCXXID) } };
                }
                else
                {
