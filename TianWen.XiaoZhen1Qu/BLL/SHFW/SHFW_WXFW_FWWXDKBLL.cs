            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", SHFW_WXFW_FWWXDKJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = SHFW_WXFW_FWWXDKJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadSHFW_WXFW_FWWXDKJBXX(string SHFW_WXFW_FWWXDKJBXXID)
        {
            try
            {
                ...
                if (SHFW_WXFW_FWWXDKJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { SHFW_WXFW_FWWXDKJBXX = SHFW_WXFW_FWWXDKJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(SHFW_WXFW_FWWXDKJBXX.JCXXID) } };
                }
                else
                {
