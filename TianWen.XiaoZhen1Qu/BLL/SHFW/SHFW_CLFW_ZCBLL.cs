            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", SHFW_CLFW_ZCJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = SHFW_CLFW_ZCJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadSHFW_CLFW_ZCJBXX(string SHFW_CLFW_ZCJBXXID)
        {
            try
            {
                ...
                if (SHFW_CLFW_ZCJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { SHFW_CLFW_ZCJBXX = SHFW_CLFW_ZCJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(SHFW_CLFW_ZCJBXX.JCXXID) } };
                }
                else
                {
