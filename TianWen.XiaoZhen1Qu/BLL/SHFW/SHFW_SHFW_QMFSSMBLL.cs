            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", SHFW_SHFW_QMFSSMJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = SHFW_SHFW_QMFSSMJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadSHFW_SHFW_QMFSSMJBXX(string SHFW_SHFW_QMFSSMJBXXID)
        {
            try
            {
                ...
                if (SHFW_SHFW_QMFSSMJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { SHFW_SHFW_QMFSSMJBXX = SHFW_SHFW_QMFSSMJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(SHFW_SHFW_QMFSSMJBXX.JCXXID) } };
                }
                else
                {
