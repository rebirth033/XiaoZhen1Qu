            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", SWFW_WZJSTGJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = SWFW_WZJSTGJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadSWFW_WZJSTGJBXX(string SWFW_WZJSTGJBXXID)
        {
            try
            {
                ...
                if (SWFW_WZJSTGJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { SWFW_WZJSTGJBXX = SWFW_WZJSTGJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(SWFW_WZJSTGJBXX.JCXXID) } };
                }
                else
                {
