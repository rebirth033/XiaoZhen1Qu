            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", SWFW_YSBZJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = SWFW_YSBZJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadSWFW_YSBZJBXX(string SWFW_YSBZJBXXID)
        {
            try
            {
                ...
                if (SWFW_YSBZJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { SWFW_YSBZJBXX = SWFW_YSBZJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(SWFW_YSBZJBXX.JCXXID) } };
                }
                else
                {
