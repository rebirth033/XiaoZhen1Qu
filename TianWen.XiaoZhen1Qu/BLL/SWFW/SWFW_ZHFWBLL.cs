            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", SWFW_ZHFWJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = SWFW_ZHFWJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadSWFW_ZHFWJBXX(string SWFW_ZHFWJBXXID)
        {
            try
            {
                ...
                if (SWFW_ZHFWJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { SWFW_ZHFWJBXX = SWFW_ZHFWJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(SWFW_ZHFWJBXX.JCXXID) } };
                }
                else
                {
