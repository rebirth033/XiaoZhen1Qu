            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", SWFW_HYZXJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = SWFW_HYZXJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadSWFW_HYZXJBXX(string SWFW_HYZXJBXXID)
        {
            try
            {
                ...
                if (SWFW_HYZXJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { SWFW_HYZXJBXX = SWFW_HYZXJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(SWFW_HYZXJBXX.JCXXID) } };
                }
                else
                {
