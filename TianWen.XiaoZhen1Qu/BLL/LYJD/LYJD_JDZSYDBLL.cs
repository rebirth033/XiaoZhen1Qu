            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", LYJD_JDZSYDJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
                        foreach (var obj in photos)
                        {
                            obj.JCXXID = LYJD_JDZSYDJBXX.JCXXID;
                            DAO.Save(obj);
                        }
                        transaction.Commit();
        ...
        public object LoadLYJD_JDZSYDJBXX(string LYJD_JDZSYDJBXXID)
        {
            try
            {
                ...
                if (LYJD_JDZSYDJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { LYJD_JDZSYDJBXX = LYJD_JDZSYDJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(LYJD_JDZSYDJBXX.JCXXID) } };
                }
                else
                {
