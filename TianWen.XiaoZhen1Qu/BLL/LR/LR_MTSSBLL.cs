            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", LR_MTSSJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
        ...
        public object LoadLR_MTSSJBXX(string LR_MTSSJBXXID)
        {
            try
            {
                ...
                if (LR_MTSSJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { LR_MTSSJBXX = LR_MTSSJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(LR_MTSSJBXX.JCXXID) } };
                }
                else
                {
