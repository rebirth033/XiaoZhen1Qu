            ...
            else
            {
                using (ITransaction transaction = DAO.BeginTransaction())
                {
                    try
                    {
                        ...
                        DAO.Save(LR_WSJBXX);
                        //照片全删全插
                        List<PHOTOS> old_photos = DAO.GetObjectList<PHOTOS>(string.Format("FROM PHOTOS WHERE JCXXID='{0}'", LR_WSJBXX.JCXXID)).ToList();
                        foreach (var obj in old_photos)
                        {
                            DAO.Remove(obj);
                        }
        ...
        public object LoadLR_WSJBXX(string LR_WSJBXXID)
        {
            try
            {
                ...
                if (LR_WSJBXX != null)
                {
                    ...
                    return new { Result = EnResultType.Success, Message = "载入成功", Value = new { LR_WSJBXX = LR_WSJBXX, JCXX = jcxx, Photos = GetPhtosByJCXXID(LR_WSJBXX.JCXXID) } };
                }
                else
                {
