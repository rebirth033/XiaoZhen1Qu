...
public class FB_FC_SJ extends PopupWindow  implements View.OnClickListener {
    ...
    public FB_FC_SJ(Activity context, View.OnClickListener itemsOnClick) {
        ...
        mMenuView = inflater.inflate(R.layout.fb_fc_sj, null);
    ...
    private void initView(View.OnClickListener itemsOnClick) {
        ...
        metsj.setOnClickListener(itemsOnClick);
    }
    ...
    private void Input(String value) {
        if(metsj.getText().toString() == "")
            metsj.setText(value + "㎡");
        else
            metsj.setText(metsj.getText().toString().replaceAll("万元","") + value + "万元");
    }
    //删除
    private void Delete() {
        if(metsj.length() == 2)
            ...
            metsj.setText(metsj.getText().toString().substring(0, metsj.length() - 3) + "万元");
    }
}
