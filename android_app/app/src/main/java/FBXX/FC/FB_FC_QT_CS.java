...
public class FB_FC_QT_CS extends Base implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.fb_fc_qt_cs);
        initView();
    }
    ...
    public void YMTZ(String id) {
        if (id == "FB_FC_QT_LB") {
            Intent intent = new Intent(FB_FC_QT_CS.this, FB_FC_QT_LB.class);
            startActivity(intent);
            finish();//关闭当前页面
        }
        if (id == "FB_FC_QT_LX") {
            qtlxWindow = new FB_FC_QT_LX(FB_FC_QT_CS.this, qtlxOnClick);
            qtlxWindow.showAtLocation(FB_FC_QT_CS.this.findViewById(R.id.fb_fc_qt_cs), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0);
            qtlxWindow.mtvlx.setText(mtvlx.getText());
            qtlxWindow.mwvlx.setWheelItemList(WheelStyle.createQTLXString());
        }
        if (id == "FB_FC_MJ") {
            mjWindow = new FB_FC_MJ(FB_FC_QT_CS.this, mjOnClick);
            mjWindow.showAtLocation(FB_FC_QT_CS.this.findViewById(R.id.fb_fc_qt_cs), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0);
            mjWindow.metmj.setText(mtvmj.getText());
        }
        if (id == "FB_FC_SJ") {
            sjWindow = new FB_FC_SJ(FB_FC_QT_CS.this, sjOnClick);
            sjWindow.showAtLocation(FB_FC_QT_CS.this.findViewById(R.id.fb_fc_qt_cs), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0);
            sjWindow.metsj.setText(mtvsj.getText());
        }
        if (id == "FB_FC_LXRSF") {
            lxrsfWindow = new FB_FC_LXRSF(FB_FC_QT_CS.this, lxrsfOnClick);
            lxrsfWindow.showAtLocation(FB_FC_QT_CS.this.findViewById(R.id.fb_fc_qt_cs), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0);
        }
        if (id == "FB_FC_LXRLXDH") {
            lxrlxdhWindow = new FB_FC_LXRLXDH(FB_FC_QT_CS.this, lxrlxdhOnClick);
            lxrlxdhWindow.showAtLocation(FB_FC_QT_CS.this.findViewById(R.id.fb_fc_qt_cs), Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, 0);
        }
    }
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                ...
                case R.id.tvqd:
                    ...
                    fb_fc_qt.SJ = new BigDecimal(sjWindow.metsj.getText().toString().replace("万","").replace("元",""));
                    break;
            }
        }
    };
