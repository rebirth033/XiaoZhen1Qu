prompt Importing table xxlb...
set feedback off
set define off
insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (289, '小类', '写字楼', 6, 1, 'FC_XZL', 'FC_XZLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (293, '小类', '仓库/厂房/土地/车位', 8, 1, 'FC_CKCFTDCW', 'FC_CKCFTDCWJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (295, '小类', '文体/母婴/儿童', 14, 9, 'ZSJM_WTMYET', 'ZSJM_WTMYETJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (301, '小类', '工商注册', 1, 13, 'SWFW_GSZC', 'SWFW_GSZCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (302, '小类', '商标专利', 2, 13, 'SWFW_SBZL', 'SWFW_SBZLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (303, '小类', '法律咨询', 3, 13, 'SWFW_FLZX', 'SWFW_FLZXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (304, '小类', '财务会计/评估', 4, 13, 'SWFW_CWKJPG', 'SWFW_CWKJPGJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (305, '小类', '保险', 5, 13, 'SWFW_BX', 'SWFW_BXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (306, '小类', '投资担保', 6, 13, 'SWFW_TZDB', 'SWFW_TZDBJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (307, '小类', '印刷包装', 7, 13, 'SWFW_YSBZ', 'SWFW_YSBZJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (308, '小类', '喷绘招牌', 8, 13, 'SWFW_PHZP', 'SWFW_PHZPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (309, '小类', '设计策划', 9, 13, 'SWFW_SJCH', 'SWFW_SJCHJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (310, '小类', '广告传媒', 10, 13, 'SWFW_GGCM', 'SWFW_GGCMJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (311, '小类', '展会服务', 11, 13, 'SWFW_ZHFW', 'SWFW_ZHFWJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (312, '小类', '礼品定制', 12, 13, 'SWFW_LPDZ', 'SWFW_LPDZJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (313, '小类', '制卡', 13, 13, 'SWFW_ZK', 'SWFW_ZKJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (314, '小类', '翻译/速记', 14, 13, 'SWFW_FYSJ', 'SWFW_FYSJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (315, '小类', '网络布线/维护', 15, 13, 'SWFW_WLBXWH', 'SWFW_WLBXWHJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (316, '小类', '网站建设/推广', 16, 13, 'SWFW_WZJSTG', 'SWFW_WZJSTGJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (317, '小类', '咨询服务', 17, 13, 'SWFW_ZXFW', 'SWFW_ZXFWJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (319, '小类', '快递', 18, 13, 'SWFW_KD', 'SWFW_KDJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (320, '小类', '货运物流', 19, 13, 'SWFW_HYWL', 'SWFW_HYWLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (321, '小类', '货运专线', 20, 13, 'SWFW_HYZX', 'SWFW_HYZXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (322, '小类', '办公设备维修', 21, 13, 'SWFW_BGSBWX', 'SWFW_BGSBWXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (323, '小类', '租赁', 22, 13, 'SWFW_ZL', 'SWFW_ZLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (324, '小类', '代办签证/签注', 23, 13, 'SWFW_DBQZQZ', 'SWFW_DBQZQZJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (325, '小类', '建筑维修', 24, 13, 'SWFW_JZWX', 'SWFW_JZWXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (326, '小类', '机械设备维修', 25, 13, 'SWFW_JXSBWX', 'SWFW_JXSBWXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (328, '小类', '短租房/日租房', 3, 1, 'FC_DZF', 'FC_DZFJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (329, '小类', '二手房', 4, 1, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (62, '小类', '工程车', 5, 3, 'CL_GCC', 'CL_GCCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (86, '小类', '游乐园/景点票', 3, 5, 'PWKQ_YLYJDP', 'PWKQ_YLYJDPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (75, '小类', '花鸟鱼虫', 3, 4, 'CW_HNYC', 'CW_HNYCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (330, '小类', '瑜伽', 5, 214, 'LR_YJ', 'LR_YJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (236, '小类', '电工电料', 13, 10, 'PFCG_DGDL', 'PFCG_DGDLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (238, '小类', '仪表仪器', 15, 10, 'PFCG_YBYQ', 'PFCG_YBYQJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (240, '小类', '原材料', 17, 10, 'PFCG_YCL', 'PFCG_YCLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (241, '小类', '包装', 18, 10, 'PFCG_BZ', 'PFCG_BZJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (243, '小类', '图书', 20, 10, 'PFCG_TS', 'PFCG_TSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (245, '小类', '机械加工', 22, 10, 'PFCG_JXJG', 'PFCG_JXJGJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (205, '小类', '全职招聘', 1, 11, 'QZZP_QZZP', 'QZZP_QZZPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (270, '小类', '设计培训', 14, 14, 'JYPX_SJPX', 'JYPX_SJPXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (271, '小类', '管理培训', 15, 14, 'JYPX_GLPX', 'JYPX_GLPXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (273, '小类', '留学', 17, 14, 'JYPX_LX', 'JYPX_LXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (275, '小类', '艺术培训教师', 8, 14, 'JYPX_YSPXJS', 'JYPX_YSPXJSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (277, '小类', '拼班拼课', 19, 14, 'JYPX_PBPK', 'JYPX_PBPKJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (123, '小类', '汽车美容装饰', 7, 99, 'SHFW_CLFW_QCMRZS', 'SHFW_CLFW_QCMRZSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (126, '小类', '汽车改装/防护', 8, 99, 'SHFW_CLFW_QCGZFH', 'SHFW_CLFW_QCGZFHJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (1, '大类', '房产', 1, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (2, '大类', '二手物品', 2, null, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (3, '大类', '车辆买卖', 3, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (4, '大类', '宠物', 4, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (5, '大类', '票务/卡券', 5, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (7, '大类', '餐饮', 7, null, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (8, '大类', '休闲娱乐', 8, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (9, '大类', '招商加盟', 9, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (10, '大类', '批发采购', 10, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (11, '大类', '求职/招聘', 11, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (12, '大类', '生活服务', 12, null, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (13, '大类', '商务服务', 13, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (14, '大类', '教育培训', 14, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (15, '大类', '旅游酒店', 15, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (16, '大类', '装修建材', 16, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (17, '大类', '婚庆摄影', 17, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (18, '大类', '农林牧副渔', 18, null, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (19, '小类', '整租房/合租房', 1, 1, 'FC_ZZF', 'FC_ZZFJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (20, '小类', '商铺', 5, 1, 'FC_SP', 'FC_SPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (26, '小类', '手机数码', 1, 2, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (27, '小类', '家电/家具/办公', 2, 2, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (28, '小类', '母婴/服装/美容', 3, 2, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (29, '小类', '文化娱乐', 4, 2, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (30, '小类', '其它', 5, 2, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (61, '小类', '轿车', 1, 3, 'CL_JC', 'CL_JCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (85, '小类', '演出门票', 2, 5, 'PWKQ_YCMP', 'PWKQ_YCMPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (64, '小类', '自行车/电动车/三轮车', 2, 3, 'CL_ZXCDDCSLC', 'CL_ZXCDDCSLCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (65, '小类', '摩托车', 3, 3, 'CL_MTC', 'CL_MTCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (68, '小类', '货车', 4, 3, 'CL_HC', 'CL_HCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (69, '小类', '客车', 6, 3, 'CL_KC', 'CL_KCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (73, '小类', '宠物狗', 1, 4, 'CW_CWG', 'CW_CWGJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (74, '小类', '宠物猫', 2, 4, 'CW_CWM', 'CW_CWMJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (78, '小类', '宠物赠送/领养', 5, 4, 'CW_CWZSLY', 'CW_CWZSLYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (79, '小类', '宠物用品/食品', 4, 4, 'CW_CWYPSP', 'CW_CWYPSPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (80, '小类', '宠物服务', 6, 4, 'CW_CWFW', 'CW_CWFWJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (82, '小类', '电影票', 1, 5, 'PWKQ_DYP', 'PWKQ_DYPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (83, '小类', '消费卡/购物券', 4, 5, 'PWKQ_XFKGWQ', 'PWKQ_XFKGWQJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (84, '小类', '其他卡券', 5, 5, 'PWKQ_QTKQ', 'PWKQ_QTKQJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (96, '小类', '生活服务', 1, 12, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (97, '小类', '维修服务', 2, 12, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (99, '小类', '车辆服务', 3, 12, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (102, '小类', '搬家', 1, 96, 'SHFW_SHFW_BJ', 'SHFW_SHFW_BJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (103, '小类', '月嫂/保姆', 2, 96, 'SHFW_SHFW_BMYS', 'SHFW_SHFW_BMYSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (104, '小类', '保洁清洗', 3, 96, 'SHFW_SHFW_BJQX', 'SHFW_SHFW_BJQXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (106, '小类', '开锁/换锁/修锁', 5, 96, 'SHFW_SHFW_KSHSXS', 'SHFW_SHFW_KSHSXSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (107, '小类', '管道疏通/清理', 6, 96, 'SHFW_SHFW_GDSTQL', 'SHFW_SHFW_GDSTQLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (108, '小类', '生活配送', 7, 96, 'SHFW_SHFW_SHPS', 'SHFW_SHFW_SHPSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (110, '小类', '起名/风水/算命', 8, 96, 'SHFW_SHFW_QMFSSM', 'SHFW_SHFW_QMFSSMJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (111, '小类', '殡葬/墓地', 9, 96, 'SHFW_SHFW_BZMD', 'SHFW_SHFW_BZMDJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (112, '小类', '家电维修', 1, 97, 'SHFW_WXFW_JDWX', 'SHFW_WXFW_JDWXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (113, '小类', '电脑维修', 2, 97, 'SHFW_WXFW_DNWX', 'SHFW_WXFW_DNWXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (114, '小类', '房屋维修/打孔', 3, 97, 'SHFW_WXFW_FWWXDK', 'SHFW_WXFW_FWWXDKJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (115, '小类', '家具维修', 4, 97, 'SHFW_WXFW_JJWX', 'SHFW_WXFW_JJWXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (116, '小类', '手机/数码维修', 5, 97, 'SHFW_WXFW_SJSMWX', 'SHFW_WXFW_SJSMWXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (119, '小类', '驾校', 3, 99, 'SHFW_CLFW_JX', 'SHFW_CLFW_JXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (120, '小类', '汽车陪练', 4, 99, 'SHFW_CLFW_QCPL', 'SHFW_CLFW_QCPLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (121, '小类', '汽修车维/保养', 5, 99, 'SHFW_CLFW_QCWXBY', 'SHFW_CLFW_QCWXBYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (122, '小类', '过户/上牌/年检/验车', 6, 99, 'SHFW_CLFW_GHSPNJYC', 'SHFW_CLFW_GHSPNJYCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (124, '小类', '租车', 1, 99, 'SHFW_CLFW_ZC', 'SHFW_CLFW_ZCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (125, '小类', '代驾/司机外派', 2, 99, 'SHFW_CLFW_DJSJWP', 'SHFW_CLFW_DJSJWPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (159, '小类', '餐饮', 1, 9, 'ZSJM_CY', 'ZSJM_CYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (160, '小类', '服装鞋包', 2, 9, 'ZSJM_FZXB', 'ZSJM_FZXBJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (161, '小类', '建材', 3, 9, 'ZSJM_JC', 'ZSJM_JCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (162, '小类', '机械', 4, 9, 'ZSJM_JX', 'ZSJM_JXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (163, '小类', '美容保健', 5, 9, 'ZSJM_MRBJ', 'ZSJM_MRBJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (164, '小类', '礼品/小商品', 6, 9, 'ZSJM_LPXSP', 'ZSJM_LPXSPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (165, '小类', '家居环保', 7, 9, 'ZSJM_JJHB', 'ZSJM_JJHBJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (166, '小类', '教育培训', 8, 9, 'ZSJM_JYPX', 'ZSJM_JYPXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (167, '小类', '汽车服务', 9, 9, 'ZSJM_QCFW', 'ZSJM_QCFWJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (168, '小类', '网络服务', 10, 9, 'ZSJM_WLFW', 'ZSJM_WLFWJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (169, '小类', '农业', 11, 9, 'ZSJM_NY', 'ZSJM_NYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (170, '小类', '特色', 12, 9, 'ZSJM_TS', 'ZSJM_TSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (171, '小类', '生活服务', 13, 9, 'ZSJM_SHFW', 'ZSJM_SHFWJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (172, '小类', '运动健身', 1, 8, 'XXYL_YDJS', 'XXYL_YDJSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (173, '小类', '夜店/酒吧/迪厅/KTV', 2, 8, 'XXYL_YDJB', 'XXYL_YDJBJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (175, '小类', '户外', 4, 8, 'XXYL_HW', 'XXYL_HWJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (176, '小类', '洗浴温泉', 5, 8, 'XXYL_XYWQ', 'XXYL_XYWQJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (177, '小类', '足疗按摩', 6, 8, 'XXYL_ZLAM', 'XXYL_ZLAMJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (178, '小类', '台球厅', 7, 8, 'XXYL_TQT', 'XXYL_TQTJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (179, '小类', '棋牌桌游', 8, 8, 'XXYL_QPZY', 'XXYL_QPZYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (180, '小类', 'DIY手工坊', 9, 8, 'XXYL_DIYSGF', 'XXYL_DIYSGFJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (182, '小类', '轰趴馆', 10, 8, 'XXYL_HPG', 'XXYL_HPGJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (186, '小类', '二手手机', 1, 26, 'ES_SJSM_ESSJ', 'ES_SJSM_ESSJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (187, '小类', '笔记本电脑', 2, 26, 'ES_SJSM_BJBDN', 'ES_SJSM_BJBDNJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (188, '小类', '平板电脑', 3, 26, 'ES_SJSM_PBDN', 'ES_SJSM_PBDNJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (189, '小类', '数码产品', 4, 26, 'ES_SJSM_SMCP', 'ES_SJSM_SMCPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (190, '小类', '台式机/配件', 5, 26, 'ES_SJSM_TSJ', 'ES_SJSM_TSJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (191, '小类', '二手家电', 1, 27, 'ES_JDJJBG_ESJD', 'ES_JDJJBG_ESJDJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (192, '小类', '二手家具', 2, 27, 'ES_JDJJBG_ESJJ', 'ES_JDJJBG_ESJJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (193, '小类', '家居/日用品', 3, 27, 'ES_JDJJBG_JJRY', 'ES_JDJJBG_JJRYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (194, '小类', '办公用品/设备', 4, 27, 'ES_JDJJBG_BGSB', 'ES_JDJJBG_BGSBJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (195, '小类', '母婴/儿童用品/玩具', 1, 28, 'ES_MYFZMR_MYETYPWJ', 'ES_MYFZMR_MYETYPWJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (196, '小类', '服装/鞋帽/箱包', 2, 28, 'ES_MYFZMR_FZXMXB', 'ES_MYFZMR_FZXMXBJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (197, '小类', '美容/保健', 3, 28, 'ES_MYFZMR_MRBJ', 'ES_MYFZMR_MRBJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (198, '小类', '艺术品/收藏品', 1, 29, 'ES_WHYL_YSPSCP', 'ES_WHYL_YSPSCPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (331, '小类', '婚车租赁', 2, 17, 'HQSY_HCZL', 'HQSY_HCZLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (332, '小类', '婚宴酒店', 3, 17, 'HQSY_HYJD', 'HQSY_HYJDJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (333, '小类', '彩妆造型', 4, 17, 'HQSY_CZZX', 'HQSY_CZZXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (334, '小类', '婚庆用品', 5, 17, 'HQSY_HQYP', 'HQSY_HQYPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (335, '小类', '司仪', 6, 17, 'HQSY_SY', 'HQSY_SYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (200, '小类', '图书/音像/软件', 3, 29, 'ES_WHYL_TSYXRJ', 'ES_WHYL_TSYXRJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (201, '小类', '网游/虚拟物品', 4, 29, 'ES_WHYL_WYXNWP', 'ES_WHYL_WYXNWPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (202, '小类', '二手设备', 1, 30, 'ES_QTES_ESSB', 'ES_QTES_ESSBJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (204, '小类', '成人用品', 2, 30, 'ES_QTES_CRYP', 'ES_QTES_CRYPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (206, '小类', '兼职招聘', 2, 11, 'QZZP_JZZP', 'QZZP_JZZPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (208, '小类', '婚庆公司', 1, 17, 'HQSY_HQGS', 'HQSY_HQGSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (209, '小类', '摄影/摄像', 24, 13, 'SWFW_SYSX', 'SWFW_SYSXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (210, '小类', '礼仪庆典', 25, 13, 'SWFW_LYQD', 'SWFW_LYQDJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (211, '小类', '美食', 1, 7, 'CY_MS', 'CY_MSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (212, '小类', '快餐/团膳', 2, 7, 'CY_KCTS', 'CY_KCTSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (213, '小类', '外卖', 3, 7, 'CY_WM', 'CY_WMJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (214, '大类', '丽人', 8, 0, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (215, '小类', '美体瘦身', 1, 214, 'LR_MTSS', 'LR_MTSSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (216, '小类', '美容护肤', 2, 214, 'LR_MRHF', 'LR_MRHFJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (217, '小类', 'SPA', 3, 214, 'LR_SPA', 'LR_SPAJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (218, '小类', '舞蹈', 4, 214, 'LR_WD', 'LR_WDJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (219, '小类', '美发护发', 6, 214, 'LR_MFHF', 'LR_MFHFJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (220, '小类', '美甲', 7, 214, 'LR_MJ', 'LR_MJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (221, '小类', '纹身', 8, 214, 'LR_WS', 'LR_WSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (222, '小类', '口腔护理', 9, 214, 'LR_KQHL', 'LR_KQHLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (223, '小类', '体检', 10, 214, 'LR_TJ', 'LR_TJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (224, '小类', '食品', 1, 10, 'PFCG_SP', 'PFCG_SPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (225, '小类', '礼品', 2, 10, 'PFCG_LP', 'PFCG_LPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (226, '小类', '服饰鞋帽', 3, 10, 'PFCG_FSXM', 'PFCG_FSXMJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (227, '小类', '箱包/饰品', 4, 10, 'PFCG_XBSP', 'PFCG_XBSPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (228, '小类', '手机数码', 5, 10, 'PFCG_SJSM', 'PFCG_SJSMJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (229, '小类', '母婴玩具', 6, 10, 'PFCG_MYWJ', 'PFCG_MYWJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (230, '小类', '户外运动', 7, 10, 'PFCG_HWYD', 'PFCG_HWYDJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (231, '小类', '化妆品', 8, 10, 'PFCG_HZP', 'PFCG_HZPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (232, '小类', '安防设备', 9, 10, 'PFCG_AFSB', 'PFCG_AFSBJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (233, '小类', '纺织/布料', 10, 10, 'PFCG_FZBL', 'PFCG_FZBLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (234, '小类', '商超设备', 11, 10, 'PFCG_SCSB', 'PFCG_SCSBJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (235, '小类', '化学品', 12, 10, 'PFCG_HXP', 'PFCG_HXPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (237, '小类', '电子元器件', 14, 10, 'PFCG_DZYQJ', 'PFCG_DZYQJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (239, '小类', '灯具照明', 16, 10, 'PFCG_DJZM', 'PFCG_DJZMJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (242, '小类', '音像', 19, 10, 'PFCG_YX', 'PFCG_YXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (244, '小类', '卡券', 21, 10, 'PFCG_KQ', 'PFCG_KQJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (246, '小类', '旅行社', 1, 15, 'LYJD_LXS', 'LYJD_LXSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (247, '小类', '国内游', 2, 15, 'LYJD_GNY', 'LYJD_GNYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (248, '小类', '周边游', 3, 15, 'LYJD_ZBY', 'LYJD_ZBYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (249, '小类', '出境游', 4, 15, 'LYJD_CJY', 'LYJD_CJYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (250, '小类', '酒店/住宿预订', 5, 15, 'LYJD_JDZSYD', 'LYJD_JDZSYDJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (251, '小类', '机票', 6, 15, 'LYJD_JP', 'LYJD_JPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (252, '小类', '导游/当地人', 7, 15, 'LYJD_DYDDR', 'LYJD_DYDDRJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (253, '小类', '家装服务', 1, 16, 'ZXJC_JZFW', 'ZXJC_JZFWJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (254, '小类', '工装服务', 2, 16, 'ZXJC_GZFW', 'ZXJC_GZFWJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (255, '小类', '房屋改造', 3, 16, 'ZXJC_FWGZ', 'ZXJC_FWGZJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (256, '小类', '建材', 4, 16, 'ZXJC_JC', 'ZXJC_JCJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (257, '小类', '家具', 5, 16, 'ZXJC_JJ', 'ZXJC_JJJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (258, '小类', '家纺家饰', 6, 16, 'ZXJC_JFJS', 'ZXJC_JFJSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (259, '小类', '中小学辅导班', 1, 14, 'JYPX_ZXXFDB', 'JYPX_ZXXFDBJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (260, '小类', '家教个人', 4, 14, 'JYPX_JJGR', 'JYPX_JJGRJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (261, '小类', '家教机构', 3, 14, 'JYPX_JJJG', 'JYPX_JJJGJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (262, '小类', '中小学一对一', 2, 14, 'JYPX_ZXXYDY', 'JYPX_ZXXYDYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (263, '小类', '语言培训', 5, 14, 'JYPX_YYPXJG', 'JYPX_YYPXJGJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (264, '小类', '语言培训教师', 6, 14, 'JYPX_YYPXJS', 'JYPX_YYPXJSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (265, '小类', '职业技能培训', 9, 14, 'JYPX_ZYJNPX', 'JYPX_ZYJNPXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (266, '小类', '学历教育', 12, 14, 'JYPX_XLJY', 'JYPX_XLJYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (267, '小类', '艺术培训', 7, 14, 'JYPX_YSPX', 'JYPX_YSPXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (268, '小类', '体育培训', 10, 14, 'JYPX_TYPX', 'JYPX_TYPXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (269, '小类', 'IT培训', 13, 14, 'JYPX_ITPX', 'JYPX_ITPXJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (272, '小类', '婴幼儿教育', 16, 14, 'JYPX_YYEJY', 'JYPX_YYEJYJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (274, '小类', '移民', 18, 14, 'JYPX_YM', 'JYPX_YMJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (276, '小类', '体育教练', 11, 14, 'JYPX_TYJL', 'JYPX_TYJLJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (278, '小类', '园林花卉', 1, 18, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (279, '小类', '农作物', 2, 18, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (280, '小类', '动植物种苗', 3, 18, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (281, '小类', '畜禽养殖', 4, 18, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (282, '小类', '水产', 5, 18, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (283, '小类', '农药/肥料', 6, 18, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (284, '小类', '饲料/兽药', 7, 18, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (285, '小类', '农机具/设备', 8, 18, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (286, '小类', '农产品加工/代理', 9, 18, null, null);

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (199, '小类', '文体/户外/乐器', 2, 29, 'ES_WHYL_WTHWYQ', 'ES_WHYL_WTHWYQJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (336, '小类', '婚礼跟拍', 7, 17, 'HQSY_HLGP', 'HQSY_HLGPJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (337, '小类', '婚纱礼服', 8, 17, 'HQSY_HSLF', 'HQSY_HSLFJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (338, '小类', '珠宝首饰', 9, 17, 'HQSY_ZBSS', 'HQSY_ZBSSJBXX');

insert into xxlb (LBID, LBLX, LBNAME, LBORDER, PARENTID, FBYM, FBTABLE)
values (339, '小类', '婚纱摄影', 10, 17, 'HQSY_HSSY', 'HQSY_HSSYJBXX');

prompt Done.
