...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/PFCG/LoadPFCG_HXPJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.PFCG_HXPJBXX.ID);
                //设置编辑器的内容
                ...
                if (xml.Value.PFCG_HXPJBXX.FWFW !== null)
                    SetDuoX("FWFW", xml.Value.PFCG_HXPJBXX.FWFW);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/PFCG/FBPFCG_HXPJBXX",
        dataType: "json",
        data:
        {
