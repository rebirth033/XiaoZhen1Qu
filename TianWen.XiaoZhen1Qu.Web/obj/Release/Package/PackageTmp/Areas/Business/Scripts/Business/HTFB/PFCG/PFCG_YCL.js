...
function PDLB(name, codeid) {
    if (name.indexOf("化工原料") !== -1 || name.indexOf("橡胶制品") !== -1) {
...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/PFCG/LoadPFCG_YCLJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                LoadXL(xml.Value.PFCG_YCLJBXX.LB, xml.Value.PFCG_YCLJBXX.XL);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/PFCG/FBPFCG_YCLJBXX",
        dataType: "json",
        data:
        {
