    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                //LoadCNXH("ESXX_PWKQ_MPKQ");
                LoadGRXX(xml.grxxlist[0]);
                //LoadJJRTJFY("ESXX_PWKQ_MPKQ");
                LoadXGLM();
            }
        },
...
function LoadJBXX(obj) {
    ...
    html += ('<span class="span_body_left_body_right_right">' + (obj.XL === null ? "暂未数据" : obj.XL) + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
...
function LoadCNXHInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'ESXX_PWKQ_MPKQ\',\'' + obj.ID + '\')" class="li_body_left_body_cnxh">');
...
function LoadJPTJInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'ESXX_PWKQ_MPKQ\',\'' + obj.ID + '\')" class="li_body_left_body_jptj">');
...
function LoadJJRTJFYInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'ESXX_PWKQ_MPKQ\',\'' + obj.ID + '\')" class="li_body_right_jjrtj">');
...
function OpenSS(TYPE, ID) {
    window.open(getRootPath() + "/Business/ESCX/ESCX_PWKQ_MPKQ?LBID=13" + "&" + TYPE + "=" + ID);
}