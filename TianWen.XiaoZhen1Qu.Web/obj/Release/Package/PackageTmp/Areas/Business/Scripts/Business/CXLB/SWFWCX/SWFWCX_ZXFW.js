...
function LoadSWFWCondition() {
    LoadConditionByTypeNames("'咨询服务类别'", "CODES_SWFW", "类别", "LB", "100");
}
//加载URL查询条件
function LoadURLCondition() {
    if (getUrlParam("LB") !== null)
        ...
        LoadBody("SWFWXX_ZXFW", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    ...
    LoadBody("SWFWXX_ZXFW", currentIndex);
    ShowSelectCondition("SWFWXX_ZXFW");
}
//选择URL条件
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'SWFWXX_ZXFW\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'SWFWXX_ZXFW\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'SWFWXX_ZXFW\',\'' + obj.ID + '\')" class="li_body_right">');
