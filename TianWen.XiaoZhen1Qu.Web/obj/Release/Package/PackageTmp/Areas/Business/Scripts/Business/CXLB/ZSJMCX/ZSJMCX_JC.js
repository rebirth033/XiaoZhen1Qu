...
function LoadZSJMCondition() {
    LoadConditionByTypeNames("'建材类别','投资金额'", "CODES_ZSJM", "类别,投资金额", "LB,TZJE", "100,100");
}
//加载URL查询条件
function LoadURLCondition() {
    if (getUrlParam("LB") !== null) {
        ...
        LoadBody("ZSJMXX_JC", currentIndex);
}
//选择URL条件
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'ZSJMXX_JC\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'ZSJMXX_JC\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'ZSJMXX_JC\',\'' + obj.ID + '\')" class="li_body_right">');
