...
$(document).ready(function () {$("body").bind("click", function () { Close("_XZQ"); Close("CX"); Close("PP"); Close("CCNX"); Close("CCYF"); Close("QY"); Close("DD"); });LoadNLMFY_SCJBXX();
    BindClick("LB");
    BindClick("QY");
    BindClick("DD");
});
...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("水产", "LB", "CODES_NLMFY");
        }
        if (type === "XL") {
            LoadXL();
...
//加载农林牧副渔_水产基本信息
function LoadNLMFY_SCJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/NLMFY_SC/LoadNLMFY_SCJBXX",
        dataType: "json",
        data:
        {
            NLMFY_SCJBXXID: getUrlParam("NLMFY_SCJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                var jsonObj = new JsonDB("myTabContent");
                jsonObj.DisplayFromJson("myTabContent", xml.Value.NLMFY_SCJBXX);
                jsonObj.DisplayFromJson("myTabContent", xml.Value.JCXX);
                $("#NLMFY_SCJBXXID").val(xml.Value.NLMFY_SCJBXX.NLMFY_SCJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
                ...
                $("#spanQY").html(xml.Value.NLMFY_SCJBXX.QY);
                $("#spanDD").html(xml.Value.NLMFY_SCJBXX.DD);
...
function FB() {
    ...
    if (getUrlParam("NLMFY_SCJBXXID") !== null)
        obj = jsonObj.AddJson(obj, "NLMFY_SCJBXXID", "'" + getUrlParam("NLMFY_SCJBXXID") + "'");

    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/NLMFY_SC/FB",
        dataType: "json",
        data:
        {
