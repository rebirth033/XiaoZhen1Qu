...
$(document).ready(function () {$("body").bind("click", function () { Close("_XZQ"); Close("CX"); Close("PP"); Close("CCNX"); Close("CCYF"); Close("QY"); Close("DD"); });LoadNLMFY_DZWZMJBXX();
    BindClick("LB");
    BindClick("QY");
    BindClick("DD");
});
...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("动植物种苗", "LB", "CODES_NLMFY");
        }
        if (type === "XL") {
            LoadXL();
...
//加载农林牧副渔_动植物种苗基本信息
function LoadNLMFY_DZWZMJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/NLMFY_DZWZM/LoadNLMFY_DZWZMJBXX",
        dataType: "json",
        data:
        {
            NLMFY_DZWZMJBXXID: getUrlParam("NLMFY_DZWZMJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                var jsonObj = new JsonDB("myTabContent");
                jsonObj.DisplayFromJson("myTabContent", xml.Value.NLMFY_DZWZMJBXX);
                jsonObj.DisplayFromJson("myTabContent", xml.Value.JCXX);
                $("#NLMFY_DZWZMJBXXID").val(xml.Value.NLMFY_DZWZMJBXX.NLMFY_DZWZMJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
                    ue.setContent(xml.Value.BCMSString);
                });
                $("#spanLB").html(xml.Value.NLMFY_DZWZMJBXX.LB);
                $("#spanXL").html(xml.Value.NLMFY_DZWZMJBXX.XL);
                $("#spanQY").html(xml.Value.NLMFY_DZWZMJBXX.QY);
                $("#spanDD").html(xml.Value.NLMFY_DZWZMJBXX.DD);
...
function FB() {
    ...
    if (getUrlParam("NLMFY_DZWZMJBXXID") !== null)
        obj = jsonObj.AddJson(obj, "NLMFY_DZWZMJBXXID", "'" + getUrlParam("NLMFY_DZWZMJBXXID") + "'");

    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/NLMFY_DZWZM/FB",
        dataType: "json",
        data:
        {
