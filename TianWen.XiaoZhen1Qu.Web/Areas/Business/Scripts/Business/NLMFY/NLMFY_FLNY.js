...
$(document).ready(function () {$("body").bind("click", function () { Close("_XZQ"); Close("CX"); Close("PP"); Close("CCNX"); Close("CCYF"); Close("QY"); Close("DD"); });LoadNLMFY_FLNYJBXX();
    BindClick("LB");
    BindClick("QY");
    BindClick("DD");
});
...
//加载农林牧副渔_肥料/农药基本信息
function LoadNLMFY_FLNYJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/NLMFY_FLNY/LoadNLMFY_FLNYJBXX",
        dataType: "json",
        data:
        {
            NLMFY_FLNYJBXXID: getUrlParam("NLMFY_FLNYJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#NLMFY_FLNYJBXXID").val(xml.Value.NLMFY_FLNYJBXX.NLMFY_FLNYJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
                ...
                $("#spanXL").html(xml.Value.NLMFY_FLNYJBXX.XL);
...
function FB() {
    ...
    if (getUrlParam("NLMFY_FLNYJBXXID") !== null)
        obj = jsonObj.AddJson(obj, "NLMFY_FLNYJBXXID", "'" + getUrlParam("NLMFY_FLNYJBXXID") + "'");

    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/NLMFY_FLNY/FB",
        dataType: "json",
        data:
        {
