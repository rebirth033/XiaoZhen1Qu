...
$(document).ready(function () {$("body").bind("click", function () { Close("_XZQ"); Close("CX"); Close("PP"); Close("CCNX"); Close("CCYF"); Close("QY"); Close("DD"); });LoadNLMFY_SLSYJBXX();
    BindClick("LB");
    BindClick("QY");
    BindClick("DD");
});
...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("饲料/兽药", "LB", "CODES_NLMFY");
        }
        if (type === "XL") {
            LoadXL();
...
function LoadNLMFY_SLSYJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/NLMFY_SLSY/LoadNLMFY_SLSYJBXX",
        dataType: "json",
        data:
        {
            NLMFY_SLSYJBXXID: getUrlParam("NLMFY_SLSYJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#NLMFY_SLSYJBXXID").val(xml.Value.NLMFY_SLSYJBXX.NLMFY_SLSYJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
                ...
                $("#spanXL").html(xml.Value.NLMFY_SLSYJBXX.XL);
...
function FB() {
    ...
    if (getUrlParam("NLMFY_SLSYJBXXID") !== null)
        obj = jsonObj.AddJson(obj, "NLMFY_SLSYJBXXID", "'" + getUrlParam("NLMFY_SLSYJBXXID") + "'");

    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/NLMFY_SLSY/FB",
        dataType: "json",
        data:
        {
