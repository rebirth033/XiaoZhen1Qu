...
function LoadTCLS(type) {
    ...
    for (var i = 0; i < colors.length; i++) {
        ...
        $("#div_tcys_right").append('<div class="div_clys"><span class="span_clys_left" style="background-color: ' + obj[0] + '"></span><span class="span_clys_right">' + obj[1] + '</span></div>');
    }
    ...
    for (var i = 0; i < colors.length; i++) {
        ...
        if (i === colors.length - 1)
            $("#div_" + type + "ys_right2").append('<div class="div_clys"><span class="span_clys_left" style="border:none;"></span><span class="span_clys_right">' + obj[0] + '</span></div>');
        else
            $("#div_" + type + "ys_right2").append('<div class="div_clys"><span class="span_clys_left" style="background-color: ' + obj[0] + '"></span><span class="span_clys_right">' + obj[1] + '</span></div>');
    }
...
function LoadHCPP() {
    ...
    $("#div_content_tcbq").html(html);
    $(".divstep_yx").bind("click", JCBQActive);
}
//婚车品牌标签切换
function JCBQActive() {
    LoadHCPPMC("婚车品牌", this.id);
}
//加载婚车品牌名称
function LoadHCPPMC(JCLX, JCBQ) {
    $.ajax({
        type: "POST",
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                for (var i = 0; i < xml.list.length; i++) {
                    html += '<span class="span_yxmc" onclick="HCPPXZ(\'' + xml.list[i].CODENAME + '\',\'' + xml.list[i].CODEID + '\')">' + xml.list[i].CODENAME + '</span>';
                }
...
//选择婚车品牌名称
function HCPPXZ(HCPPMC, HCPPID) {
    $("#spanTCPP").html(HCPPMC);
...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadDropdown("婚车租赁", "LB");
        }
        if (type === "TCPP") {
            LoadHCPP();
            LoadHCPPMC("婚车品牌", "divA");
        }
        if (type === "XL") {
            LoadXL();
        }
    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                if (xml.Value.HQSY_HCZLJBXX.TGFW !== null)
                    SetDuoX("TGFW", xml.Value.HQSY_HCZLJBXX.TGFW);
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) { //有错误时的回调函数
...
function FB() {
    ...
    obj = jsonObj.AddJson(obj, "TGFW", "'" + GetDuoX("TGFW") + "'");
