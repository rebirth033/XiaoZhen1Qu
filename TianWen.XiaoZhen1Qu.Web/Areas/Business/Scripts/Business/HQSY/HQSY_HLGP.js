...
$(document).ready(function () {$("body").bind("click", function () { Close("_XZQ"); Close("CX"); Close("PP"); Close("CCNX"); Close("CCYF"); Close("QY"); Close("DD"); });BindClick("GPLX");
    BindClick("QY");
    BindClick("DD");
    LoadHQSY_HLGPJBXX();
});
...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "GPLX") {
            LoadCODESByTYPENAME("跟拍类型", "GPLX", "CODES_HQSY");
        }
        if (type === "XL") {
            LoadXL();
        }
...
function LoadHQSY_HLGPJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/HQSY_HLGP/LoadHQSY_HLGPJBXX",
        dataType: "json",
        data:
        {
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/HQSY_HLGP/FB",
        dataType: "json",
        data:
        {
