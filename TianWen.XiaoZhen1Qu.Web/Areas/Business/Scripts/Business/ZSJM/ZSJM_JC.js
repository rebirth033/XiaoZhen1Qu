...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("建材", "LB", "CODES_ZSJM");
        }
        if (type === "PPLS") {
...
//加载招商加盟_建材基本信息
function LoadZSJM_JCJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/ZSJM_JC/LoadZSJM_JCJBXX",
        dataType: "json",
        data:
        {
            ZSJM_JCJBXXID: getUrlParam("ZSJM_JCJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ZSJM_JCJBXXID").val(xml.Value.ZSJM_JCJBXX.ZSJM_JCJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
                    ue.setContent(xml.Value.ZSJM_JCJBXX.BCMS);
                });
                ...
                SetSHRQ(xml.Value.ZSJM_JCJBXX.SHRQ);
                SetZSDQ(xml.Value.ZSJM_JCJBXX.ZSDQ);
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) { //有错误时的回调函数
...
function FB() {
    ...
    if (getUrlParam("ZSJM_JCJBXXID") !== null)
        obj = jsonObj.AddJson(obj, "ZSJM_JCJBXXID", "'" + getUrlParam("ZSJM_JCJBXXID") + "'");

    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/ZSJM_JC/FB",
        dataType: "json",
        data:
        {
