    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                var html = "<ul class='ulFWPZ'>";
                for (var i = 0; i < xml.list.length; i++) {
                    html += "<li class='liJYPXXL' onclick='SelectDuoX(this)'><img class='img_JYPXXL'/><label style='font-weight:normal;'>" + xml.list[i].CODENAME + "</label></li>";
                    if (i === 5 || i === 11 || i === 17 || i === 23 || i === 29) {
                        html += "</ul><ul class='ulFWPZ' style='margin-left: 214px'>";
                    }
                }
    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                if (xml.Value.ZSJM_JYPXJBXX.SHRQ !== null)
                    SetDuoX("SHRQ", xml.Value.ZSJM_JYPXJBXX.SHRQ);
