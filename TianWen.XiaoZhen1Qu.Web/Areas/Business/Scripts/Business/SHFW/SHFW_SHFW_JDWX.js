...
//加载生活服务_家电维修基本信息
function LoadSHFW_SHFW_JDWXJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SHFW_SHFW_JDWX/LoadSHFW_SHFW_JDWXJBXX",
        dataType: "json",
        data:
        {
            SHFW_SHFW_JDWXJBXXID: getUrlParam("SHFW_SHFW_JDWXJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                var jsonObj = new JsonDB("myTabContent");
                jsonObj.DisplayFromJson("myTabContent", xml.Value.SHFW_SHFW_JDWXJBXX);
                jsonObj.DisplayFromJson("myTabContent", xml.Value.JCXX);
                $("#SHFW_SHFW_JDWXJBXXID").val(xml.Value.SHFW_SHFW_JDWXJBXX.SHFW_SHFW_JDWXJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
                    ue.setContent(xml.Value.SHFW_SHFW_JDWXJBXX.BCMS);
                });
                $("#spanLB").html(xml.Value.SHFW_SHFW_JDWXJBXX.LB);
                $("#spanQY").html(xml.Value.SHFW_SHFW_JDWXJBXX.QY);
                $("#spanDD").html(xml.Value.SHFW_SHFW_JDWXJBXX.DD);
                SetSFSM(xml.Value.SHFW_SHFW_JDWXJBXX.SFSM);

                LoadPhotos(xml.Value.Photos);
                LoadXL(xml.Value.SHFW_SHFW_JDWXJBXX.LB, xml.Value.SHFW_SHFW_JDWXJBXX.XL);
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) { //有错误时的回调函数
...
function FB() {
    ...
    if (getUrlParam("SHFW_SHFW_JDWXJBXXID") !== null)
        obj = jsonObj.AddJson(obj, "SHFW_SHFW_JDWXJBXXID", "'" + getUrlParam("SHFW_SHFW_JDWXJBXXID") + "'");

    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SHFW_SHFW_JDWX/FB",
        dataType: "json",
        data:
        {
