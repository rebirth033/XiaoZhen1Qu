...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("家电维修", "LB", "CODES_SHFW");
        }
        if (type === "QY") {
            LoadQY();
        }
...
//加载生活服务_家电维修基本信息
function LoadSHFW_WXFW_JDWXJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SHFW_WXFW_JDWX/LoadSHFW_WXFW_JDWXJBXX",
        dataType: "json",
        data:
        {
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                ue.ready(function () {
                    ue.setHeight(200);
                    ue.setContent(xml.Value.SHFW_WXFW_JDWXJBXX.BCMS);
                });
                $("#spanLB").html(xml.Value.SHFW_WXFW_JDWXJBXX.LB);
                $("#spanQY").html(xml.Value.SHFW_WXFW_JDWXJBXX.QY);
                $("#spanDD").html(xml.Value.SHFW_WXFW_JDWXJBXX.DD);
                SetDX("SFSM", xml.Value.SHFW_WXFW_JDWXJBXX.SFSM);
                LoadPhotos(xml.Value.Photos);
                LoadXL(xml.Value.SHFW_WXFW_JDWXJBXX.LB, xml.Value.SHFW_WXFW_JDWXJBXX.XL);
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) { //有错误时的回调函数
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SHFW_WXFW_JDWX/FB",
        dataType: "json",
        data:
        {
