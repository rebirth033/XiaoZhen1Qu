...
function LoadDefault() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/ZSJMXX/LoadZSJMXX",
        dataType: "json",
        data:
        {
            TYPE: "ZSJMXX_CY",
...
function LoadJBXX(obj) {
    ...
    if (obj.XL !== null) {
        ...
        html += ('<span class="span_body_left_body_right_right">' + ValidateNull(obj.XL) + '</span>');
        html += ('</p>');
    }
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">品牌名称：</span>');
    html += ('<span class="span_body_left_body_right_right">' + obj.PPMC + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">品牌历史：</span>');
    html += ('<span class="span_body_left_body_right_right">' + HandlerData(obj.PPLS) + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">投资金额：</span>');
    html += ('<span class="span_body_left_body_right_right">' + HandlerData(obj.TZJE) + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left" style="width:90px !important;margin-left:0px !important;">全国分店数：</span>');
    html += ('<span class="span_body_left_body_right_right">' + HandlerData(obj.QGFDS) + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">单店面积：</span>');
    html += ('<span class="span_body_left_body_right_right">' + HandlerData(obj.DDMJ) + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
...
function LoadXQ(obj, BCMSString) {
    ...
    for (var i = 0; i < obj.PHOTOS.length; i++) {
        ...
        html += ('<img id="img_body_left_body_xq_xx' + i + '" onload="DrawImage(this,405,350)" onclick="ImgShow(this)" class="img_body_left_body_xq_xx" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[i].PHOTONAME + "?j=" + Math.random() + '" />');
        html += ('</li>');
    }
    html += ('</ul>');
    html += ('</div>');
...
function LoadCNXH(TYPE) {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/ZSJMCX/LoadZSJMXX",
        dataType: "json",
        data:
        {
            TYPE: TYPE,
...
function LoadCNXHInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'ZSJMXX_CY\',\'' + obj.ID + '\')" class="li_body_left_body_cnxh">');
    ...
    html += ('<p class="p_li_body_left_body_cnxh_cs">' + obj.FWFW + '</p>');
    html += ('</li>');
    return html;
}
//加载精品推荐
...
function LoadJPTJInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'ZSJMXX_CY\',\'' + obj.ID + '\')" class="li_body_left_body_jptj">');
    ...
    html += ('<p class="p_li_body_left_body_jptj_cs">' + obj.FWFW + '</p>');
    html += ('</li>');
    return html;
}
//加载该经纪人推荐
...
function LoadJJRTJFYInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'ZSJMXX_CY\',\'' + obj.ID + '\')" class="li_body_right_jjrtj">');
    ...
    html += ('<p class="p_li_body_right_jjrtj_cs">' + obj.FWFW + '</p>');
    html += ('</div>');
    html += ('</li>');
    return html;
}
    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                for (var i = 0; i < xml.list.length; i++) {
                    if (xml.list[i].FBYM.indexOf("ZSJM_") !== -1)
...
function OpenXGLM(lbname, lbid) {
    if (lbname.indexOf("ZSJM_") !== -1)
        window.open(getRootPath() + "/" + "/ZSJMCX/" + lbname.replace("ZSJM_", "ZSJMCX_") + "?LBID=" + lbid);
}
//搜索栏备注导航
function OpenSS(TYPE, ID) {
    window.open(getRootPath() + "/ZSJMCX/ZSJMCX_JDJJBG_BGSB?LBID=13" + "&" + TYPE + "=" + ID);
}