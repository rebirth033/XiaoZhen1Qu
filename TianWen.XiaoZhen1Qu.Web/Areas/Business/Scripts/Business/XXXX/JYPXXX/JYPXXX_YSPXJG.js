...
function LoadDefault() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/JYPXXX/LoadJYPXXX",
        dataType: "json",
        data:
        {
            TYPE: "JYPXXX_YSPXJG",
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                LoadCNXH("JYPXXX_YSPXJG");
                LoadGRXX(xml.grxxlist[0]);
                LoadJJRTJFY("JYPXXX_YSPXJG");
                HandlerTPXX();
            }
        },
...
function LoadJBXX(obj) {
    ...
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">对&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;象：</span>');
    html += ('<span class="span_body_left_body_right_right">' + obj.DX + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    ...
    html += ('</p>');
    html += ('<p class="p_body_left_body_right" style="width:700px;">');
    ...
    html += ('</p>');
    html += ('<div style="margin-top:20px;" class="div_body_left_body_right_lxdh" id="div_body_left_body_right_lxdh" onclick="ShowLXDH(\'' + obj.LXDH + '\')"><img class="img_body_left_body_right_lxdh"  src="' + getRootPath() + "/Areas/Business/Css/images/XXXX/GY/xxxx_gy_lxdh.png" + '" /><span class="span_body_left_body_right_lxdh">联系电话</span></div>');
    html += ('</div>');
    html += ('</div>');
...
function LoadCNXHInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'JYPXXX_YSPXJG\',\'' + obj.ID + '\')" class="li_body_left_body_cnxh">');
...
function LoadJPTJInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'JYPXXX_YSPXJG\',\'' + obj.ID + '\')" class="li_body_left_body_jptj">');
...
function LoadJJRTJFYInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'JYPXXX_YSPXJG\',\'' + obj.ID + '\')" class="li_body_right_jjrtj">');
    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                for (var i = 0; i < xml.list.length; i++) {
                    if (xml.list[i].FBYM.indexOf("JYPX_") !== -1)
...
function OpenXGLM(lbname, lbid) {
    if (lbname.indexOf("JYPX_") !== -1)
        window.open(getRootPath() + "/" + "/JYPXCX/" + lbname.replace("JYPX_", "JYPXCX_") + "?LBID=" + lbid);
}
//搜索栏备注导航
function OpenSS(TYPE, ID) {
    window.open(getRootPath() + "/JYPXCX/JYPXCX_JDJJBG_BGSB?LBID=13" + "&" + TYPE + "=" + ID);
}