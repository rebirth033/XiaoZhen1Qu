...
function LoadDefault() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/HQSYXX/LoadHQSYXX",
        dataType: "json",
        data:
        {
            TYPE: "HQSYXX_HCZL",
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                LoadCNXH("HQSYXX_HCZL");
                LoadGRXX(xml.grxxlist[0]);
                LoadJJRTJFY("HQSYXX_HCZL");
                HandlerTPXX();
            }
        },
...
function LoadJBXX(obj) {
    ...
    html += ('<span class="span_body_left_body_right_left">头车品牌：</span>');
    html += ('<span class="span_body_left_body_right_right">' + obj.TCPP + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">头车颜色：</span>');
    html += ('<span class="span_body_left_body_right_right">' + obj.TCYS + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">跟车品牌：</span>');
    html += ('<span class="span_body_left_body_right_right">' + obj.GCPP + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">跟车颜色：</span>');
    html += ('<span class="span_body_left_body_right_right">' + obj.GCYS + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">跟车数量：</span>');
    html += ('<span class="span_body_left_body_right_right">' + obj.GCSL + '辆</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">提供车花：</span>');
    html += ('<span class="span_body_left_body_right_right">' + obj.GCSL + '(免费)</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
...
function LoadCNXHInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'HQSYXX_HCZL\',\'' + obj.ID + '\')" class="li_body_left_body_cnxh">');
...
function LoadJPTJInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'HQSYXX_HCZL\',\'' + obj.ID + '\')" class="li_body_left_body_jptj">');
...
function LoadJJRTJFYInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'HQSYXX_HCZL\',\'' + obj.ID + '\')" class="li_body_right_jjrtj">');
    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                for (var i = 0; i < xml.list.length; i++) {
                    if (xml.list[i].FBYM.indexOf("HQSY_") !== -1)
...
function OpenXGLM(lbname, lbid) {
    if (lbname.indexOf("HQSY_") !== -1)
        window.open(getRootPath() + "/" + "/HQSYCX/" + lbname.replace("HQSY_", "HQSYCX_") + "?LBID=" + lbid);
}
//搜索栏备注导航
function OpenSS(TYPE, ID) {
    window.open(getRootPath() + "/HQSYCX/HQSYCX_JDJJBG_BGSB?LBID=13" + "&" + TYPE + "=" + ID);
}