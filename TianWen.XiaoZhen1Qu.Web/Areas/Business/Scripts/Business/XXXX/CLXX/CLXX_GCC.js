...
function LoadDefault() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/CLXX/LoadCLXX",
        dataType: "json",
        data:
        {
            TYPE: "CLXX_GCC",
...
function LoadJBXX(obj) {
    ...
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">小时数：</span>');
    html += ('<span class="span_body_left_body_right_right">' + obj.XSS + '小时</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
    html += ('<span class="span_body_left_body_right_left">吨位：</span>');
...
function OpenSS(TYPE, ID) {
    window.open(getRootPath() + "/Business/CLCX/CLCX_KC?LBID=13" + "&" + TYPE + "=" + ID);
}