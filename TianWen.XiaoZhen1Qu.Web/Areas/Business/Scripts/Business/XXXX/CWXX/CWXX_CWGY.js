...
function LoadDefault() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/CWXX/LoadCWXX",
        dataType: "json",
        data:
        {
            TYPE: "CWXX_CWGY",
...
function LoadJBXX(obj) {
    ...
    html += ('<span class="span_body_left_body_right_left">供求：</span>');
...
function OpenXGLM(lbname, lbid) {
    if (lbname.indexOf("CW_") !== -1)
        window.open(getRootPath() + "/Business" + "/CWCX/" + lbname.replace("CW_", "CWCX_") + "?LBID=" + lbid);
}
//搜索栏备注导航
function OpenSS(TYPE, ID) {
    window.open(getRootPath() + "/Business/CWCX/CWCX_CWGY?LBID=13" + "&" + TYPE + "=" + ID);
}