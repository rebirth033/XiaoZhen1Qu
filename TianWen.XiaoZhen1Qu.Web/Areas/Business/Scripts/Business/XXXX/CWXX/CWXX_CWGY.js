...
function LoadJBXX(obj) {
    ...
    html += ('<span class="span_body_left_body_right_left">供&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;求：</span>');
    html += ('<span class="span_body_left_body_right_right">' + obj.GQ + '</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
...
function LoadCNXHInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'CWXX_CWGY\',\'' + obj.ID + '\')" class="li_body_left_body_cnxh">');
    ...
    html += ('<p class="p_li_body_left_body_cnxh_cs">' + obj.GQ + '</p>');
    html += ('</li>');
    return html;
}
//加载精品推荐
...
function LoadJPTJInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'CWXX_CWGY\',\'' + obj.ID + '\')" class="li_body_left_body_jptj">');
    ...
    html += ('<p class="p_li_body_left_body_jptj_cs">' + obj.GQ + '</p>');
    html += ('</li>');
    return html;
}
//加载该经纪人推荐
...
function LoadJJRTJFYInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'CWXX_CWGY\',\'' + obj.ID + '\')" class="li_body_right_jjrtj">');
    ...
    html += ('<p class="p_li_body_right_jjrtj_cs">' + obj.GQ + '</p>');
    html += ('</div>');
    html += ('</li>');
    return html;
}
    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                for (var i = 0; i < xml.list.length; i++) {
                    if (xml.list[i].FBYM.indexOf("CW_") !== -1)
...
function OpenXGLM(lbname, lbid) {
    if (lbname.indexOf("CW_") !== -1)
        window.open(getRootPath() + "/" + "/CWCX/" + lbname.replace("CW_", "CWCX_") + "?LBID=" + lbid);
}
//搜索栏备注导航
function OpenSS(TYPE, ID) {
    window.open(getRootPath() + "/CWCX/CWCX_CWGY?LBID=13" + "&" + TYPE + "=" + ID);
}