...
function LoadDefault() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/SHFWXX/LoadSHFWXX",
        dataType: "json",
        data:
        {
            TYPE: "SHFWXX_KSXSHS",
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                LoadCNXH("SHFWXX_KSXSHS");
                LoadGRXX(xml.grxxlist[0]);
                LoadJJRTJFY("SHFWXX_KSXSHS");
                HandlerTPXX();
            }
        },
...
function LoadCNXHInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'SHFWXX_KSXSHS\',\'' + obj.ID + '\')" class="li_body_left_body_cnxh">');
...
function LoadJPTJInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'SHFWXX_KSXSHS\',\'' + obj.ID + '\')" class="li_body_left_body_jptj">');
...
function LoadJJRTJFYInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'SHFWXX_KSXSHS\',\'' + obj.ID + '\')" class="li_body_right_jjrtj">');
    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                for (var i = 0; i < xml.list.length; i++) {
                    if (xml.list[i].FBYM.indexOf("SHFW_") !== -1)
...
function OpenXGLM(lbname, lbid) {
    if (lbname.indexOf("SHFW_") !== -1)
        window.open(getRootPath() + "/" + "/SHFWCX/" + lbname.replace("SHFW_", "SHFWCX_") + "?LBID=" + lbid);
}
//搜索栏备注导航
function OpenSS(TYPE, ID) {
    window.open(getRootPath() + "/SHFWCX/SHFWCX_JDJJBG_BGSB?LBID=13" + "&" + TYPE + "=" + ID);
}