...
function LoadDefault() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/PFCGXX/LoadPFCGXX",
        dataType: "json",
        data:
        {
            TYPE: "PFCGXX_DZYQJ",
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                LoadCNXH("PFCGXX_DZYQJ");
                LoadGRXX(xml.grxxlist[0]);
                LoadJJRTJFY("PFCGXX_DZYQJ");
                HandlerTPXX();
            }
        },
...
function LoadCNXHInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'PFCGXX_DZYQJ\',\'' + obj.ID + '\')" class="li_body_left_body_cnxh">');
...
function LoadJPTJInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'PFCGXX_DZYQJ\',\'' + obj.ID + '\')" class="li_body_left_body_jptj">');
...
function LoadJJRTJFYInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'PFCGXX_DZYQJ\',\'' + obj.ID + '\')" class="li_body_right_jjrtj">');
...
function OpenXGLM(lbname, lbid) {
    if (lbname.indexOf("PFCG_") !== -1)
        window.open(getRootPath() + "/" + "/PFCGCX/" + lbname.replace("PFCG_", "PFCGCX_") + "?LBID=" + lbid);
}
//搜索栏备注导航
function OpenSS(TYPE, ID) {
    window.open(getRootPath() + "/PFCGCX/PFCGCX_JDJJBG_BGSB?LBID=13" + "&" + TYPE + "=" + ID);
}