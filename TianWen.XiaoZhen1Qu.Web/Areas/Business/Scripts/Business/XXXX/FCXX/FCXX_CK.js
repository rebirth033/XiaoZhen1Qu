    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                LoadCNXH("FCXX_CK", xml.list[0].GQ);
                LoadGRXX(xml.grxxlist[0]);
                LoadJJRTJFY("FCXX_CK", xml.list[0].GQ);
                HandlerTPXX();
            }
        },
...
function LoadJBXX(obj) {
    ...
    html += ('<span class="span_body_left_body_right_left">房屋类别：</span>');
    html += ('<span class="span_body_left_body_right_right">仓库</span>');
    html += ('</p>');
    html += ('<p class="p_body_left_body_right">');
...
function LoadXQ(obj, BCMSString) {
    ...
    html += ('<p class="p_body_left_body_xq">仓库详情</p>');
