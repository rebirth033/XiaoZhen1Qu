...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("投资担保类别", "LB", "CODES_SWFW", Bind, "OUTLB", "LB");
        }
    });
}
//选择类别下拉框
...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/SWFW/LoadSWFW_TZDBJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.SWFW_TZDBJBXX.ID);
                //设置编辑器的内容
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/SWFW/FBSWFW_TZDBJBXX",
        dataType: "json",
        data:
        {
