...
//判断类别
function PDLB(lbmc) {
    if (lbmc === "空调租赁" || lbmc === "户外装备租赁" || lbmc === "移动厕所租赁") {
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                for (var i = 0; i < xml.list.length; i++) {
                    html += "<li class='liXL' style='width:150px;' onclick='SelectDuoX(this)'><img class='img_XL'/><label style='font-weight:normal;'>" + xml.list[i].CODENAME + "</label></li>";
                    if (i % 4 === 3) {
...
function LoadSWFW_ZLJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW/LoadSWFW_ZLJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW/FBSWFW_ZLJBXX",
        dataType: "json",
        data:
        {
