...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("商标专利类别", "LB", "CODES_SWFW", Bind, "OUTLB", "LB", "");
        }
    });
}
//选择类别下拉框
...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/SWFW/LoadSWFW_SBZLJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.SWFW_SBZLJBXX.ID);
                //设置编辑器的内容
                ...
                SetDX("SFSM", xml.Value.SWFW_SBZLJBXX.SFSM);
                if (xml.Value.SWFW_SBZLJBXX.FWFW !== null)
                    SetDuoX("FWFW", xml.Value.SWFW_SBZLJBXX.FWFW);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/SWFW/FBSWFW_SBZLJBXX",
        dataType: "json",
        data:
        {
