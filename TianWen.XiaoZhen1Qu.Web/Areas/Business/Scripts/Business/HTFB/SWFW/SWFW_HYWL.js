...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/SWFW/LoadSWFW_HYWLJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                SetDuoX("LB", xml.Value.SWFW_HYWLJBXX.LB);
                $("#spanQY").html(xml.Value.SWFW_HYWLJBXX.QY);
                $("#spanDD").html(xml.Value.SWFW_HYWLJBXX.DD);
                if (xml.Value.SWFW_HYWLJBXX.FWFW !== null)
                    SetDuoX("FWFW", xml.Value.SWFW_HYWLJBXX.FWFW);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/SWFW/FBSWFW_HYWLJBXX",
        dataType: "json",
        data:
        {
