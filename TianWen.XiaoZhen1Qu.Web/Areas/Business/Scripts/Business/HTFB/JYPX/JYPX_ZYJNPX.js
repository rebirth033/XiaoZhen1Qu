...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("职业技能培训类别", "LB", "CODES_JYPX", Bind, "OUTLB", "LB", "");
        }
    });
}
//加载商务服务_职业技能培训基本信息
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/JYPX/LoadJYPX_ZYJNPXJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.JYPX_ZYJNPXJBXX.ID);
                //设置编辑器的内容
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/JYPX/FBJYPX_ZYJNPXJBXX",
        dataType: "json",
        data:
        {
