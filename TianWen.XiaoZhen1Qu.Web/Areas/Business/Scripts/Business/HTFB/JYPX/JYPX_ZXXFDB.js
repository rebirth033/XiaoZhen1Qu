    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                if (parseInt(xml.list.length % 4) === 0)
                    $("#divSKXS").css("height", parseInt(xml.list.length / 4) * 40 + "px");
                else
                    $("#divSKXS").css("height", (parseInt(xml.list.length / 4) + 1) * 40 + "px");
                html += "</ul>";
...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/JYPX/LoadJYPX_ZXXFDBJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/JYPX/FBJYPX_ZXXFDBJBXX",
        dataType: "json",
        data:
        {
