...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/CY/LoadCY_MSJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.CY_MSJBXX.ID);
                //设置编辑器的内容
                ue.ready(function () { ue.setContent(xml.Value.BCMSString); });
                if (xml.Value.CY_MSJBXX.LB !== null)
                    SetDuoX("MSLB", xml.Value.CY_MSJBXX.LB);
                $("#spanQY").html(xml.Value.CY_MSJBXX.QY);
                $("#spanDD").html(xml.Value.CY_MSJBXX.DD);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/CY/FBCY_MSJBXX",
        dataType: "json",
        data:
        {
