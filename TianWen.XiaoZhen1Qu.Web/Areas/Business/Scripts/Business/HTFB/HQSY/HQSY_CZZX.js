...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/HQSY/LoadHQSY_CZZXJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                if (xml.Value.HQSY_CZZXJBXX.FWFW !== null)
                    SetDuoX("FWFW", xml.Value.HQSY_CZZXJBXX.FWFW);
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) { //有错误时的回调函数
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/HQSY/FBHQSY_CZZXJBXX",
        dataType: "json",
        data:
        {
