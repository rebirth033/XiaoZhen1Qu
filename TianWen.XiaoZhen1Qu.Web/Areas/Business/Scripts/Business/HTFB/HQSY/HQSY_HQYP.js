...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/HQSY/LoadHQSY_HQYPJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.HQSY_HQYPJBXX.ID);
                //设置编辑器的内容
                ...
                if (xml.Value.HQSY_HQYPJBXX.FWFW !== null)
                    SetDuoX("FWFW", xml.Value.HQSY_HQYPJBXX.FWFW);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/HQSY/FBHQSY_HQYPJBXX",
        dataType: "json",
        data:
        {
