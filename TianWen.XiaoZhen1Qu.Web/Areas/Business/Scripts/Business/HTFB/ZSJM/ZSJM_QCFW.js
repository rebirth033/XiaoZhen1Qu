...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("汽车服务类别", "LB", "CODES_ZSJM", Bind, "QCFWLB", "LB", "");
        }
        if (type === "PPLS") {
...
function LoadZSJM_QCFWJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/ZSJM/LoadZSJM_QCFWJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.ZSJM_QCFWJBXX.ID);
                //设置编辑器的内容
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/ZSJM/FBZSJM_QCFWJBXX",
        dataType: "json",
        data:
        {
