...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("环保产品类别", "LB", "CODES_ZSJM", Bind, "CYLB", "LB", "");
        }
        if (type === "PPLS") {
...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/ZSJM/LoadZSJM_JJHBJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.ZSJM_JJHBJBXX.ID);
                //设置编辑器的内容
                ...
                if (xml.Value.ZSJM_JJHBJBXX.LB.indexOf("饰品挂件") !== -1) {
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/ZSJM/FBZSJM_JJHBJBXX",
        dataType: "json",
        data:
        {
