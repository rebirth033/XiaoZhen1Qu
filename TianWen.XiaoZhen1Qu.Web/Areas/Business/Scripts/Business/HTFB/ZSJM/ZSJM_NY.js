...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/ZSJM/LoadZSJM_NYJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.ZSJM_NYJBXX.ID);
                //设置编辑器的内容
                ...
                if (xml.Value.ZSJM_NYJBXX.SHRQ !== null)
                    SetDuoX("SHRQ", xml.Value.ZSJM_NYJBXX.SHRQ);
                if (xml.Value.ZSJM_NYJBXX.JYMS !== null)
                    SetDuoX("JYMS", xml.Value.ZSJM_NYJBXX.JYMS);
                if (xml.Value.ZSJM_NYJBXX.FWFW !== null)
                    SetDuoX("FWFW", xml.Value.ZSJM_NYJBXX.FWFW);
                if (xml.Value.ZSJM_NYJBXX.LB.indexOf("种植养殖") !== -1) {
                    LoadXLByName(xml.Value.ZSJM_NYJBXX.LB, xml.Value.ZSJM_NYJBXX.XL,"CODES_ZSJM");
                }
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) { //有错误时的回调函数
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/ZSJM/FBZSJM_NYJBXX",
        dataType: "json",
        data:
        {
