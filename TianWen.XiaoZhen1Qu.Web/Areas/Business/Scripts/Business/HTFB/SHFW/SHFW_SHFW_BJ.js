...
function LoadSHFW_SHFW_BJJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SHFW/LoadSHFW_SHFW_BJJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.SHFW_SHFW_BJJBXX.ID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SHFW/FBSHFW_SHFW_BJJBXX",
        dataType: "json",
        data:
        {
