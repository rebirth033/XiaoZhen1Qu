...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("游乐园/景点票类别", "LB", "CODES_ES_PWKQ", Bind, "MPKQLB", "LB", "");
        }
    });
}
//加载票务卡券_电影票基本信息
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/ES/LoadES_PWKQ_MPKQJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/ES/FBES_PWKQ_MPKQJBXX",
        dataType: "json",
        data:
        {
