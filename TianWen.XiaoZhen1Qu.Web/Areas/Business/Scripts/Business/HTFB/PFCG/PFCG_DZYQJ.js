...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/PFCG/LoadPFCG_DZYQJJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.PFCG_DZYQJJBXX.ID);
                //设置编辑器的内容
                ...
                if (xml.Value.PFCG_DZYQJJBXX.FWFW !== null)
                    SetDuoX("FWFW", xml.Value.PFCG_DZYQJJBXX.FWFW);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/PFCG/FBPFCG_DZYQJJBXX",
        dataType: "json",
        data:
        {
