...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/PFCG/LoadPFCG_TSJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.PFCG_TSJBXX.ID);
                //设置编辑器的内容
                ue.ready(function () { ue.setContent(xml.Value.BCMSString); });
                $("#spanQY").html(xml.Value.PFCG_TSJBXX.QY);
                $("#spanDD").html(xml.Value.PFCG_TSJBXX.DD);
                if (xml.Value.PFCG_TSJBXX.FWFW !== null)
                    SetDuoX("FWFW", xml.Value.PFCG_TSJBXX.FWFW);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/PFCG/FBPFCG_TSJBXX",
        dataType: "json",
        data:
        {
