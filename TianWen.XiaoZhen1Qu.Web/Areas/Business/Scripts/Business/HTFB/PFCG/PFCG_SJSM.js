...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("手机数码类别", "LB", "CODES_PFCG", Bind, "OUTLB", "LB", "");
        }
    });
}
//选择类别下拉框
...
function LoadJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/PFCG/LoadPFCG_SJSMJBXX",
        dataType: "json",
        data:
        {
            ID: getUrlParam("ID")
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#ID").val(xml.Value.PFCG_SJSMJBXX.ID);
                //设置编辑器的内容
                ...
                if (xml.Value.PFCG_SJSMJBXX.FWFW !== null)
                    SetDuoX("FWFW", xml.Value.PFCG_SJSMJBXX.FWFW);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/PFCG/FBPFCG_SJSMJBXX",
        dataType: "json",
        data:
        {
