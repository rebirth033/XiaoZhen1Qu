...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("网站建设/推广", "LB", "CODES_CL");
        }
        if (type === "QY") {
            LoadQY();
        }
...
function LoadSWFW_WZJSTGJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW_WZJSTG/LoadSWFW_WZJSTGJBXX",
        dataType: "json",
        data:
        {
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                ue.ready(function () {
                    ue.setHeight(200);
                    ue.setContent(xml.Value.SWFW_WZJSTGJBXX.BCMS);
                });
                ...
                SetSFSM(xml.Value.SWFW_WZJSTGJBXX.SFSM);
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW_WZJSTG/FB",
        dataType: "json",
        data:
        {
