...
$(document).ready(function () {$("body").bind("click", function () { Close("_XZQ"); Close("CX"); Close("PP"); Close("CCNX"); Close("CCYF"); Close("QY"); Close("DD"); });LoadSWFW_LYQDJBXX();
    BindClick("LB");
    BindClick("QY");
    BindClick("DD");
});
...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("礼仪庆典", "LB", "CODES_SWFW");
        }
        if (type === "XL") {
            LoadXL();
        }
...
function LoadSWFW_LYQDJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW_LYQD/LoadSWFW_LYQDJBXX",
        dataType: "json",
        data:
        {
            SWFW_LYQDJBXXID: getUrlParam("SWFW_LYQDJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#SWFW_LYQDJBXXID").val(xml.Value.SWFW_LYQDJBXX.SWFW_LYQDJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
...
function FB() {
    ...
    if (getUrlParam("SWFW_LYQDJBXXID") !== null)
        obj = jsonObj.AddJson(obj, "SWFW_LYQDJBXXID", "'" + getUrlParam("SWFW_LYQDJBXXID") + "'");

    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW_LYQD/FB",
        dataType: "json",
        data:
        {
