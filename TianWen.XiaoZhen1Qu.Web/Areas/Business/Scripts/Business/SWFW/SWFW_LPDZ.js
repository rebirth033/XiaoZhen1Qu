...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("礼品定制", "LB", "CODES_SWFW");
        }
        if (type === "QY") {
            LoadQY();
        }
...
//加载商务服务_礼品定制基本信息
function LoadSWFW_LPDZJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW_LPDZ/LoadSWFW_LPDZJBXX",
        dataType: "json",
        data:
        {
            SWFW_LPDZJBXXID: getUrlParam("SWFW_LPDZJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#SWFW_LPDZJBXXID").val(xml.Value.SWFW_LPDZJBXX.SWFW_LPDZJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
                    ue.setContent(xml.Value.SWFW_LPDZJBXX.BCMS);
                });
...
function FB() {
    ...
    if (getUrlParam("SWFW_LPDZJBXXID") !== null)
        obj = jsonObj.AddJson(obj, "SWFW_LPDZJBXXID", "'" + getUrlParam("SWFW_LPDZJBXXID") + "'");

    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW_LPDZ/FB",
        dataType: "json",
        data:
        {
