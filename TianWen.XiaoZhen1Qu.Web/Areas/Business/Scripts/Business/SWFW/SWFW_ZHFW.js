...
//判断类别
function PDLB(lbmc) {
    if (lbmc === "展位设计" || lbmc === "展会布置/搭建" || lbmc === "展会用品制作") {
        LoadXL($("#spanLB").html());
        $("#divXL").css("display", "");
    }
    else {
...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("展会服务", "LB", "CODES_SWFW", Bind, "OUTLB", "LB", "");
        }
        if (type === "QY") {
            LoadQY();
        }
...
//加载商务服务_展会服务基本信息
function LoadSWFW_ZHFWJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW/LoadSWFW_ZHFWJBXX",
        dataType: "json",
        data:
        {
            SWFW_ZHFWJBXXID: getUrlParam("SWFW_ZHFWJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                var jsonObj = new JsonDB("myTabContent");
                jsonObj.DisplayFromJson("myTabContent", xml.Value.SWFW_ZHFWJBXX);
                jsonObj.DisplayFromJson("myTabContent", xml.Value.JCXX);
                $("#SWFW_ZHFWJBXXID").val(xml.Value.SWFW_ZHFWJBXX.SWFW_ZHFWJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
...
function FB() {
    ...
    if (getUrlParam("SWFW_ZHFWJBXXID") !== null)
        obj = jsonObj.AddJson(obj, "SWFW_ZHFWJBXXID", "'" + getUrlParam("SWFW_ZHFWJBXXID") + "'");

    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW/FBSWFW_ZHFWJBXX",
        dataType: "json",
        data:
        {
