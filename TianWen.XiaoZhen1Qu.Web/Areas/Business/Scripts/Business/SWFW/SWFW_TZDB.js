...
function BindClick(type) {
    $("#div" + type + "Span").click(function () {
        if (type === "LB") {
            LoadCODESByTYPENAME("投资担保", "LB", "CODES_SWFW");
        }
        if (type === "QY") {
            LoadQY();
        }
...
function LoadSWFW_TZDBJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW_TZDB/LoadSWFW_TZDBJBXX",
        dataType: "json",
        data:
        {
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                ue.ready(function () {
                    ue.setHeight(200);
                    ue.setContent(xml.Value.SWFW_TZDBJBXX.BCMS);
                });
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/SWFW_TZDB/FB",
        dataType: "json",
        data:
        {
