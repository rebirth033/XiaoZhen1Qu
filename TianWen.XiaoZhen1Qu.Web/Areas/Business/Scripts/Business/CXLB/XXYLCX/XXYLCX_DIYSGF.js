...
$(document).ready(function () {
    BindBodyNav();
    LoadXXYLCondition();
    LoadHot("XXYLXX_DIYSGF");
});
//加载条件
function LoadXXYLCondition() {
    LoadConditionByTypeName("DIY手工坊", "CODES_XXYL", "类别", "LB", 15);
...
function SelectCondition(obj, name) {
    ...
    LoadBody("XXYLXX_DIYSGF", currentIndex);
    ShowSelectCondition("XXYLXX_DIYSGF");
}
//加载主体部分
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'XXYLXX_DIYSGF\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'XXYLXX_DIYSGF\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
