...
function LoadXXYLCondition() {
    LoadConditionByTypeName("夜店酒吧", "CODES_XXYL", "类别", "LB", 15);
    LoadDistrict("福州", "350100", "QY");
    LoadBody("XXYLXX_YDJB", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    ...
    LoadBody("XXYLXX_YDJB", currentIndex);
    ShowSelectCondition("XXYLXX_YDJB");
}
//加载主体部分
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'XXYLXX_YDJB\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'XXYLXX_YDJB\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
