...
function LoadXXYLCondition() {
    LoadConditionByTypeNames("'休闲娱乐类别'", "CODES_XXYL", "类别", "OTHER", "15");
    LoadBody("XXYLXX_KTV", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    ...
    LoadBody("XXYLXX_KTV", currentIndex);
    ShowSelectCondition("XXYLXX_KTV");
}
//加载主体部分
function LoadBody(TYPE, PageIndex) {
    currentIndex = parseInt(PageIndex);
    var condition = GetAllCondition("QY");
    $.ajax({
        type: "POST",
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'XXYLXX_KTV\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'XXYLXX_KTV\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
