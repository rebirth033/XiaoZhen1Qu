...
function LoadCWCondition() {
    ...
    var lb = "类别,不限,宠物医院,宠物寄养,宠物美容,宠物训练,宠物托运,宠物配种,宠物殡葬,宠物摄影,水族服务,其他宠物服务".split(',');
...
function LoadBody(TYPE, PageIndex) {
    ...
    var condition = "LB:" + GetCondition("LB") + ",DQ:" + GetCondition("DQ");
    $.ajax({
        type: "POST",
...
function LoadCWInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'CWXX_CWFW\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'CWXX_CWFW\',\'' + obj.ID + '\')">' + TruncStr(obj.BT,35) + '</p>');
