...
$(document).ready(function () {
    ...
    LoadCWCondition();
    LoadHot("CWXX_CWZSLY");
    LoadBody("CWXX_CWZSLY", currentIndex);
});
//绑定主体列表导航
...
function LoadCWCondition() {
    ...
    var gq = "供需,不限,宠物赠送,宠物领养,寻宠启事".split(',');
    var dq = "地区,不限,鼓楼,台江,晋安,仓山,闽侯,福清,马尾,长乐,连江,平潭,罗源,闽清,永泰".split(',');
    LoadCondition(gq, "GQ");
    LoadCondition(dq, "DQ");
    $("#ul_condition_body_GQ").find(".li_condition_body").bind("click", SelectCondition);
...
function LoadBody(TYPE, PageIndex) {
    ...
    var condition = "GQ:" + GetCondition("GQ") + ",DQ:" + GetCondition("DQ");
    $.ajax({
        type: "POST",
...
function LoadCWInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'CWXX_CWZSLY\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'CWXX_CWZSLY\',\'' + obj.ID + '\')">' + TruncStr(obj.BT, 35) + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'CWXX_CWZSLY\',\'' + obj.ID + '\')" class="li_body_right">');
