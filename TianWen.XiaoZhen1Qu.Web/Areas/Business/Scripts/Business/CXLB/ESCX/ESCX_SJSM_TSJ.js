...
function LoadESCondition() {
    ...
    var lb = "类别,不限,台式机,服务器,硬件,外设产品,网络设备,上网套餐,电脑回收".split(',');
    var cpupp = "CPU品牌,不限,酷睿,酷睿2,酷睿i5,速龙,酷睿i3,酷睿i7,奔腾,赛扬,FX,APU,闪龙".split(',');
    ...
    LoadCondition(lb, "LB");
    LoadCondition(cpupp, "CPUPP");
    LoadCondition(jg, "JG");
    LoadCondition(dq, "DQ");
    $("#ul_condition_body_LB").find(".li_condition_body").bind("click", SelectCondition);
    $("#ul_condition_body_CPUPP").find(".li_condition_body").bind("click", SelectCondition);
...
function LoadBody(TYPE, PageIndex) {
    ...
    var condition = "LB:" + GetCondition("LB") + ",CPUPP:" + GetCondition("CPUPP") + ",JG:" + GetCondition("JG") + ",DQ:" + GetCondition("DQ");
    $.ajax({
        type: "POST",
...
function LoadESInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'ESXX_SJSM_TSJ\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'ESXX_SJSM_TSJ\',\'' + obj.ID + '\')">' + TruncStr(obj.BT,35) + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'ESXX_SJSM_TSJ\',\'' + obj.ID + '\')" class="li_body_right">');
