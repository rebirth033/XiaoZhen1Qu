...
function LoadESCondition() {
    ...
    var lb = "类别,不限,相机/配件,耳机耳麦,存储设备,移动电源,电子书/kindle,智能设备,游戏机,MP3/MP4,数码摄像机,其他数码,音箱/音响,其他智能设备".split(',');
    var xl = "小类,不限,单反相机,数码相机,微单相机,镜头,闪光灯,脚架/云台,存储卡/读卡器,相机电池/电源,相机包".split(',');
...
function LoadBody(TYPE, PageIndex) {
    ...
    var condition = "LB:" + GetCondition("LB") + ",XL:" + GetCondition("XL") + ",JG:" + GetCondition("JG") + ",DQ:" + GetCondition("DQ");
    $.ajax({
        type: "POST",
...
function LoadESInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'ESXX_SJSM_SMCP\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'ESXX_SJSM_SMCP\',\'' + obj.ID + '\')">' + TruncStr(obj.BT,35) + '</p>');
