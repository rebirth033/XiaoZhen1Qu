...
$(document).ready(function () {
    BindBodyNav();
    LoadESCondition();
    LoadHot("ESXX_WHYL_YSPSCP");
});
//加载条件
function LoadESCondition() {
    LoadConditionByTypeNames("'艺术品/收藏品类别','艺术品价格'", "CODES_ES_WHYL", "类别,价格", "LB,JG", "100,100");
}
//加载URL查询条件
function LoadURLCondition() {
    if (getUrlParam("JG") !== null)
        ...
        LoadBody("ESXX_WHYL_YSPSCP", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    ...
    LoadBody("ESXX_WHYL_YSPSCP", currentIndex);
    ShowSelectCondition("ESXX_WHYL_YSPSCP");
}
//选择URL条件
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'ESXX_WHYL_YSPSCP\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'ESXX_WHYL_YSPSCP\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'ESXX_WHYL_YSPSCP\',\'' + obj.ID + '\')" class="li_body_right">');
