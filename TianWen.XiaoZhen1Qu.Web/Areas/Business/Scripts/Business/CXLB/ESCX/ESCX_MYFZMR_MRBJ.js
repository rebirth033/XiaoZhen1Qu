...
function LoadESCondition() {
    ...
    var lb = "类别,不限,彩妆,面部护肤,美容工具,美容设施,纤体/美体,香水,美发,美甲,保健食品,保健器械,口腔护理".split(',');
    var xl = "小类,不限,洁面,化妆水,爽肤水,乳液,面霜,精华,面膜,眼部护理,精油".split(',');
    var jg = "价格,不限,100元以下,100-150元,150-300元,300-500元,500-1000元,1000-2000元,2000元以上".split(',');
...
function LoadBody(TYPE, PageIndex) {
    ...
    var condition = "LB:" + GetCondition("LB") + ",JG:" + GetCondition("JG") + ",DQ:" + GetCondition("DQ");
    $.ajax({
        type: "POST",
...
function LoadESInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'ESXX_MYFZMR_MRBJ\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'ESXX_MYFZMR_MRBJ\',\'' + obj.ID + '\')">' + TruncStr(obj.BT,35) + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'ESXX_MYFZMR_MRBJ\',\'' + obj.ID + '\')" class="li_body_right">');
