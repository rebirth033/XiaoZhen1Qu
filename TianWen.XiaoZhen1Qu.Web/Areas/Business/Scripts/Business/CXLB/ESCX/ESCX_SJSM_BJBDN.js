...
function LoadESCondition() {
    LoadConditionByTypeName("笔记本类别", "CODES_ES_SJSM", "类别", "LB");
...
function SelectCondition(obj, name) {
    ...
    if (name === "类别" && obj.innerHTML === "笔记本") {
        ...
        LoadConditionByTypeName("笔记本品牌", "CODES_ES_SJSM", "品牌", "BJBPP");
    }
...
function LoadBody(TYPE, PageIndex) {
    ...
    var condition = GetAllCondition("LB,XL,BJBPP,JG,QY");
    $.ajax({
        type: "POST",
...
function LoadESInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'ESXX_SJSM_BJBDN\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'ESXX_SJSM_BJBDN\',\'' + obj.ID + '\')">' + TruncStr(obj.BT,35) + '</p>');
