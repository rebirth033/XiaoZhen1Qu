...
function LoadPFCGCondition() {
    LoadConditionByTypeNames("'箱包/饰品类别'", "CODES_PFCG", "类别", "LB", "15");
...
function SelectCondition(obj, name) {
    ...
    LoadBody("PFCGXX_XBSP", currentIndex);
    ShowSelectCondition("PFCGXX_XBSP");
}
//加载主体部分
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'PFCGXX_XBSP\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'PFCGXX_XBSP\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'PFCGXX_XBSP\',\'' + obj.ID + '\')" class="li_body_right">');
