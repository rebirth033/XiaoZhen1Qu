...
function LoadPFCGCondition() {
    LoadConditionByTypeNames("'安防设备类别'", "CODES_PFCG", "类别", "LB", "100");
}
//加载URL查询条件
function LoadURLCondition() {
    if (getUrlParam("LB") !== null) {
        ...
        LoadBody("PFCGXX_AFSB", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    ...
    LoadBody("PFCGXX_AFSB", currentIndex);
    ShowSelectCondition("PFCGXX_AFSB");
}
//选择URL条件
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'PFCGXX_AFSB\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'PFCGXX_AFSB\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
