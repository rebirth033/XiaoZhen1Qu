...
function LoadPFCGCondition() {
    LoadConditionByTypeName("音像", "CODES_PFCG", "类别", "LB", 15);
    LoadDistrict("福州", "350100", "QY");
    LoadBody("PFCGXX_YX", currentIndex);
}
//选择条件
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'PFCGXX_YX\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'PFCGXX_YX\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
