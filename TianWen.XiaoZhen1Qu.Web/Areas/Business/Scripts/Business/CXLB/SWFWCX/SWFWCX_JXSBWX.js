...
$(document).ready(function () {
    BindBodyNav();
    LoadSWFWCondition();
    LoadHot("SWFWCX_JXSBWX");
});
//加载条件
function LoadSWFWCondition() {
    LoadConditionByTypeName("机械设备维修", "CODES_SWFW", "类别", "LB", 15);
...
function SelectCondition(obj, name) {
    ...
    LoadBody("SWFWCX_JXSBWX", currentIndex);
    ShowSelectCondition("SWFWCX_JXSBWX");
}
//加载主体部分
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'SWFWCX_JXSBWX\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'SWFWCX_JXSBWX\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
