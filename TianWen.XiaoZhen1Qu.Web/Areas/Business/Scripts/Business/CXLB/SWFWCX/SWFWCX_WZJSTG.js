...
$(document).ready(function () {
    BindBodyNav();
    LoadSWFWCondition();
    LoadHot("SWFWXX_WZJSTG");
});
//加载条件
function LoadSWFWCondition() {
    LoadConditionByTypeName("网站建设/推广", "CODES_SWFW", "类别", "LB", 15);
...
function SelectCondition(obj, name) {
    ...
    LoadBody("SWFWXX_WZJSTG", currentIndex);
    ShowSelectCondition("SWFWXX_WZJSTG");
}
//加载主体部分
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'SWFWXX_WZJSTG\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'SWFWXX_WZJSTG\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
