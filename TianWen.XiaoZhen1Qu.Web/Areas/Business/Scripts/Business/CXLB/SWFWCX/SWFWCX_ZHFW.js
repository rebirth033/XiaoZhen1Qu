...
function LoadSWFWCondition() {
    LoadConditionByTypeNames("'展会服务类别'", "CODES_SWFW", "类别", "LB", "100");
}
//加载URL查询条件
function LoadURLCondition() {
    if (getUrlParam("LB") !== null)
        ...
        LoadBody("SWFWXX_ZHFW", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    if (name === "类别" && (obj.innerHTML !== "展会庆典策划" && obj.innerHTML !== "摄影摄像")) {
    ...
    LoadBody("SWFWXX_ZHFW", currentIndex);
    ShowSelectCondition("SWFWXX_ZHFW");
}
//选择URL条件
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'SWFWXX_ZHFW\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'SWFWXX_ZHFW\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'SWFWXX_ZHFW\',\'' + obj.ID + '\')" class="li_body_right">');
