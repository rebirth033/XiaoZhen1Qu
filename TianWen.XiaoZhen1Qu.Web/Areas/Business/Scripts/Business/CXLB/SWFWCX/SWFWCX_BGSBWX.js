...
$(document).ready(function () {
    BindBodyNav();
    LoadSWFWCondition();
    LoadHot("SWFWXX_BGSBWX");
});
//加载条件
function LoadSWFWCondition() {
    LoadConditionByTypeName("办公设备维修", "CODES_SWFW", "类别", "LB", 15);
    LoadConditionByTypeName("是否上门", "CODES_SWFW", "是否上门", "SFSM", 15);
...
function SelectCondition(obj, name) {
    ...
    LoadBody("SWFWXX_BGSBWX", currentIndex);
    ShowSelectCondition("SWFWXX_BGSBWX");
}
//加载主体部分
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'SWFWXX_BGSBWX\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'SWFWXX_BGSBWX\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
