...
function LoadCondition(array, name, id, length) {
    ...
    if (name === "语种")
        html += '<li id="li_condition_body_more" class="li_condition_body" onclick="MoreYZ()" style="color:#2274e0;">全部语种<img id="span_select_arrow_blue" class="span_select_arrow_blue" /></li>';
    html += '</ul>';
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                for (var i = 0; i < BQArray.length; i++) {
                    ...
                    for (var j = 0; j < xml.list.length; j++) {
                        if (BQArray[i] === xml.list[j].CODEVALUE)
                            html += '<li id="' + xml.list[j].CODEID + '" onclick="SelectYZCondition(this)" class="li_row_right_jcpp_first_right_value">' + xml.list[j].CODENAME + '</li>';
                    }
                    html += '</ul>';
                }
...
//选择品种条件
function SelectYZCondition(obj) {

    $("#ul_condition_body_YZ").find(".li_condition_body").each(function () {
        $(this).removeClass("li_condition_body_active");
    });
    ...
    if (hasPP === 0)
        $("#ul_condition_body_YZ").append('<li class="li_condition_body li_condition_body_more" id="' + obj.id + '" onclick="SelectCondition(this,\'语种\')">' + obj.innerHTML + '</li>');
    $("#ul_condition_body_YZ").append('<li id="li_condition_body_more" class="li_condition_body" onclick="MoreYZ()" style="color:#2274e0;">全部语种<img id="span_select_arrow_blue" class="span_select_arrow_blue" /></li>');
...
function LoadBody(TYPE, PageIndex) {
    ...
    var condition = GetAllCondition("YZ,ZX,JB,QY");
    $.ajax({
        type: "POST",
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'JYPXXX_YYPXJG\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'JYPXXX_YYPXJG\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
