...
function LoadJYPXCondition() {
    LoadConditionByTypeName("体育培训", "CODES_JYPX", "类别", "LB", 15);
    LoadConditionByTypeName("体育培训对象", "CODES_JYPX", "对象", "DX");
    LoadDistrict("福州", "350100", "QY");
    LoadBody("JYPXXX_TYPX", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    ...
    LoadBody("JYPXXX_TYPX", currentIndex);
    ShowSelectCondition("JYPXXX_TYPX");
}
//加载主体部分
function LoadBody(TYPE, PageIndex) {
    currentIndex = parseInt(PageIndex);
    var condition = GetAllCondition("LB,XL,DX,QY");
    $.ajax({
        type: "POST",
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'JYPXXX_TYPX\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'JYPXXX_TYPX\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
