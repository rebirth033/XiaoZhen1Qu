...
function LoadJYPXCondition() {
    ...
    LoadBody("JYPXCX_XLJY", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    ...
    LoadBody("JYPXCX_XLJY", currentIndex);
    ShowSelectCondition("JYPXCX_XLJY");
}
//加载主体部分
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'JYPXCX_XLJY\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'JYPXCX_XLJY\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
