...
$(document).ready(function () {
    BindBodyNav();
    LoadJYPXCondition();
    LoadHot("JYPXXX_ZXXFDB");
});
//加载条件
function LoadJYPXCondition() {
    LoadConditionByTypeName("中小学辅导班", "CODES_JYPX", "类别", "LB", 15);
    LoadConditionByTypeName("授课形式", "CODES_JYPX", "授课形式", "SKXS");
    LoadDistrict("福州", "350100", "QY");
    LoadBody("JYPXXX_ZXXFDB", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    if (name === "类别" && (obj.innerHTML !== "夏冬令营")) {
    ...
    LoadBody("JYPXXX_ZXXFDB", currentIndex);
    ShowSelectCondition("JYPXXX_ZXXFDB");
}
//加载主体部分
function LoadBody(TYPE, PageIndex) {
    currentIndex = parseInt(PageIndex);
    var condition = GetAllCondition("LB,XL,SKXS,QY");
    $.ajax({
        type: "POST",
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'JYPXXX_ZXXFDB\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'JYPXXX_ZXXFDB\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
