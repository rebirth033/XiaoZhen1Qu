...
function LoadLYJDCondition() {
    LoadConditionByTypeNames("'签证类别'", "CODES_LYJD", "类别", "LB", "100");
}
//加载URL查询条件
function LoadURLCondition() {
    if (getUrlParam("LB") !== null)
        ...
        LoadBody("LYJDXX_QZFW", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    ...
    LoadBody("LYJDXX_QZFW", currentIndex);
    ShowSelectCondition("LYJDXX_QZFW");
}
//选择URL条件
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'LYJDXX_QZFW\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'LYJDXX_QZFW\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'LYJDXX_QZFW\',\'' + obj.ID + '\')" class="li_body_right">');
