...
function LoadLYJDCondition() {
    LoadConditionByTypeNames("'旅行社'", "CODES_LYJD", "旅行社", "LXS", "15");
    LoadBody("LYJDXX_LXS", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    ...
    LoadBody("LYJDXX_LXS", currentIndex);
    ShowSelectCondition("LYJDXX_LXS");
}
//加载主体部分
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'LYJDXX_LXS\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'LYJDXX_LXS\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
