...
function LoadLYJDCondition() {
    LoadConditionByTypeNames("'国内游游玩天数','出游方式','国内游价格'", "CODES_LYJD", "游玩天数,出游方式,价格", "XCTS_R,CYFS,JG_CR", "100,100,100");
}
//加载URL查询条件
...
function LoadBody(TYPE, PageIndex) {
    ...
    var condition = GetAllCondition("XCTS_R,CYFS,CYLB,JG_CR,QY");
    $.ajax({
        type: "POST",
...
function LoadInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'LYJDXX_GNY\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'LYJDXX_GNY\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'LYJDXX_GNY\',\'' + obj.ID + '\')" class="li_body_right">');
