...
function LoadHQSYCondition() {
    ...
    LoadConditionByTypeName("婚礼跟拍类型", "CODES_HQSY", "跟拍类型", "GPLX");
...
function LoadBody(TYPE, PageIndex) {
    ...
    var condition = GetAllCondition("JG,GPLX,QY");
    $.ajax({
        type: "POST",
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'HQSYXX_HLGP\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'HQSYXX_HLGP\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
