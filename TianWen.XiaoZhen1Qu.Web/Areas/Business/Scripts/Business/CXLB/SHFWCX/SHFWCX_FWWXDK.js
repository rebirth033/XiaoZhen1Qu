...
$(document).ready(function () {
    BindBodyNav();
    LoadSHFWCondition();
    LoadHot("SHFWXX_FWWXDK");
});
//加载条件
function LoadSHFWCondition() {
    LoadConditionByTypeName("房屋维修", "CODES_SHFW", "类别", "LB", 15);
    LoadConditionByTypeName("是否上门", "CODES_SHFW", "是否上门", "SFSM", 15);
    LoadDistrict("福州", "350100", "QY");
    LoadBody("SHFWXX_FWWXDK", currentIndex);
}
//选择条件
function SelectCondition(obj, name) {
    ...
    LoadBody("SHFWXX_FWWXDK", currentIndex);
    ShowSelectCondition("SHFWXX_FWWXDK");
}
//加载主体部分
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'SHFWXX_FWWXDK\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'SHFWXX_FWWXDK\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
