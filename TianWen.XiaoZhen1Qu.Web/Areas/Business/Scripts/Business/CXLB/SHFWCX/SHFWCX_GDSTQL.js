...
$(document).ready(function () {
    BindBodyNav();
    LoadSHFWCondition();
    LoadHot("SHFWXX_GDSTQL");
});
//加载条件
function LoadSHFWCondition() {
    LoadConditionByTypeName("管道疏通/清理", "CODES_SHFW", "类别", "LB", 15);
...
function LoadQZZPInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'SHFWXX_GDSTQL\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'SHFWXX_GDSTQL\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
