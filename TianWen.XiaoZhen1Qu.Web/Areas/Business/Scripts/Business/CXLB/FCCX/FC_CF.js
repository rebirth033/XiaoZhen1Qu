...
$(document).ready(function () {
    ...
    BindBodyNav();
    LoadFCXX_CFCZCondition();
    LoadHot("FC_CF");
    LoadBody("FC_CF", currentIndex);
});
...
//加载出租条件
function LoadFCXX_CFCZCondition() {
...
//加载出售条件
function LoadFCXX_CFCSCondition() {
    $("#div_condition_body").html('');
    var dq = "地区,不限,鼓楼,台江,晋安,仓山,闽侯,福清,马尾,长乐,连江,平潭,罗源,闽清,永泰".split(',');
    var fl = "类型,不限,写字楼,商务中心".split(',');
...
function LoadCZInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'FC_CF\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'FC_CF\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
...
function LoadHotInfo(obj) {
    var html = "";
    html += ('<li onclick="OpenXXXX(\'FC_CF\',\'' + obj.ID + '\')" class="li_body_right">');
