...
function LoadCSCondition() {
    RemoveCondition("QY,ZJ,SJ,MJ");
    LoadConditionByTypeNames("'土地售价','仓库面积'", "CODES_FC", "售价,面积", "ZJ,MJ");
    LoadBody("FCXX_CK", currentIndex);
}
//绑定查询条件导航
...
function LoadCZInfo(obj) {
    ...
    html += ('<img class="img_li_body_left" onclick="OpenXXXX(\'FCXX_CK\',\'' + obj.ID + '\')" src="' + getRootPath() + "/Areas/Business/Photos/" + obj.YHID + "/" + obj.PHOTOS[0].PHOTONAME + "?j=" + Math.random() + '" />');
    html += ('<div class="div_img_li_body_left_count"><span>' + obj.PHOTOS.length + '图</span></div>');
    html += ('</div>');
    html += ('<div class="div_li_body_left_center">');
    html += ('<p class="p_li_body_left_center_bt" onclick="OpenXXXX(\'FCXX_CK\',\'' + obj.ID + '\')">' + obj.BT + '</p>');
