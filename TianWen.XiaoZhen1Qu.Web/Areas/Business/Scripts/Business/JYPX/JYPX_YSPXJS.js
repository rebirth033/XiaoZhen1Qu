        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                for (var i = 0; i < xml.list.length; i++) {
                    html += '<span class="span_yxmc" onclick="BYYXXZ(\'' + xml.list[i].CODENAME + '\',\'' + xml.list[i].CODEID + '\')">' + xml.list[i].CODENAME + '</span>';
                }
    ...
    $.ajax({
        ...
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                ue.ready(function () {
                    ue.setHeight(200);
                    ue.setContent(xml.Value.JYPX_YSPXJSJBXX.BCMS);
                });
