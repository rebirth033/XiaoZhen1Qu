...
$(document).ready(function () {$("body").bind("click", function () { Close("_XZQ"); Close("CX"); Close("PP"); Close("CCNX"); Close("CCYF"); Close("QY"); Close("DD"); });BindClick("GJ");
    BindClick("QY");
    BindClick("DD");
    LoadDuoX("移民类别", "YMLB");
});
...
function LoadJYPX_YMJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/JYPX_YM/LoadJYPX_YMJBXX",
        dataType: "json",
        data:
        {
            JYPX_YMJBXXID: getUrlParam("JYPX_YMJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#JYPX_YMJBXXID").val(xml.Value.JYPX_YMJBXX.JYPX_YMJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
...
function FB() {
    ...
    if (getUrlParam("JYPX_YMJBXXID") !== null)
        obj = jsonObj.AddJson(obj, "JYPX_YMJBXXID", "'" + getUrlParam("JYPX_YMJBXXID") + "'");

    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/JYPX_YM/FB",
        dataType: "json",
        data:
        {
