...
function LoadJYPX_PBPKJBXX() {
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/JYPX_PBPK/LoadJYPX_PBPKJBXX",
        dataType: "json",
        data:
        {
            JYPX_PBPKJBXXID: getUrlParam("JYPX_PBPKJBXXID")
        },
        success: function (xml) {
            if (xml.Result === 1) {
                ...
                $("#JYPX_PBPKJBXXID").val(xml.Value.JYPX_PBPKJBXX.JYPX_PBPKJBXXID);
                //设置编辑器的内容
                ue.ready(function () {
                    ue.setHeight(200);
                    ue.setContent(xml.Value.JYPX_PBPKJBXX.BCMS);
                });
...
function FB() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath() + "/Business/JYPX_PBPK/FB",
        dataType: "json",
        data:
        {
