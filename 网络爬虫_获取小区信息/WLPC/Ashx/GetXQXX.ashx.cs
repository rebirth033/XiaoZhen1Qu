...
namespace WLPC.Ashx
{
    ...
    public class GetXQXX : IHttpHandler
    {
                ...
                for (int i = 0; i < list.Length; i++)
                {
                    ...
                    for (int j = 0; j < matches_first.Count; j++)
                    {
                        ...
                        for (int k = 1; k < 50; k++)
                        {
                            ...
                            {
                                address = GetAddress(matches_second[l].Groups["url"].Value);
                                sql = "insert into codes_xqjbxx(xqjbxxid,xqmc,xqdz,szs,szx) values(S_XQJBXX.NEXTVAL,'" + matches_second[l].Groups["text"].Value + "','" + address + "','" + szs + "','" + szx + "')";
        ...
        public string GetAddress(string url)
        {
            ...
            string pattern = "<p(\\s+(class=\"address\"|'([^'])*'|\\w+=\"(([^\"])*)\"|'([^'])*'))+>(?<text>(.*?))</p>";
            Regex regex = new Regex(pattern);
            MatchCollection childmatches = regex.Matches(pageinfo);
